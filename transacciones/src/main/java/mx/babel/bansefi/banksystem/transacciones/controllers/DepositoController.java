...
public class DepositoController implements Serializable {
    ...
    private boolean origenFichaCuenta = false;
          ...
          //SE OBTIENE EL VALOR DE LA BUSQUEDA DE CUENTAS
        	if(this.obtieneFlash().get(ParametrosFlashEnum.NUMERO_CUENTA.getParamFlash()) != null){
        		if (this.obtieneFlash().get(ParametrosFlashEnum.NAVEGACION_MANAGED_BEAN.getParamFlash()) != null 
        				&& managedBeanStateRecover.getDestino() != null 
        				&& managedBeanStateRecover.getDestino().equals(NavegacionEnum.FICHA_CUENTA)){
        			this.destino = managedBeanStateRecover.getDestino();
                    this.destinoController = managedBeanStateRecover.getController();
                    this.origenFichaCuenta = true;
        		}else{
    ...
    public String continuarDeposito() {
        ...
        if (respuesta != null && DepositoController.CTE_CERO.equals(respuesta.getEstatus())) {
        	...
        	busquedaNivelCuentaBackEnd.EjecutarTRN(this.deposito.getCuentaDeposito().toString());
            ...
            if(this.destino != null && this.destinoController != null){
            	if (destino.equals(NavegacionEnum.FICHA_CUENTA)){
            		CuentaBean cuenta = new CuentaBean();
            		cuenta.setNumeroCuenta(this.deposito.getCuentaDeposito());
            		this.obtieneFlash().put(ParametrosFlashEnum.CUENTA_BEAN.getParamFlash(), cuenta);
    ...
    public String irMenu(){
		if (this.destino != null && destino.equals(NavegacionEnum.FICHA_CUENTA)) {
			CuentaBean cuenta = new CuentaBean();
			cuenta.setNumeroCuenta(deposito.getCuentaDeposito());
			obtieneFlash().put(ParametrosFlashEnum.CUENTA_BEAN.getParamFlash(), cuenta);
    ...
    public String irDeposito(){
    	if (destino!=null && destino.equals(NavegacionEnum.FICHA_CUENTA)){
    		obtieneFlash().clear();
    		...
    		obtieneFlash().put(ParametrosFlashEnum.NIVEL_CUENTA.getParamFlash(),this.deposito.getNivelCuenta());
        	this.deposito = new DepositoBean();
    ...
    /**
     * Redireccion a ficha de cuenta
     * @return
     */
    public String irFichaCuenta(){
        CuentaBean cuenta = new CuentaBean();
        cuenta.setNumeroCuenta(deposito.getCuentaDeposito());
        obtieneFlash().put(ParametrosFlashEnum.CUENTA_BEAN.getParamFlash(), cuenta);
        return NavegacionEnum.FICHA_CUENTA.getRuta();
	...
	/**
	 * @param origenFichaCuenta the origenFichaCuenta to set
	 */
	public void setOrigenFichaCuenta(boolean origenFichaCuenta) {
