...
import mx.babel.bansefi.banksystem.contabilidad.webservices.altaapuntemanualcuenta.AltaApunteManualCuentaServicio;
import mx.babel.bansefi.banksystem.contabilidad.webservices.altaapuntemanualcuenta.Ejecutar;
import mx.babel.bansefi.banksystem.contabilidad.webservices.altaapuntemanualcuenta.EjecutarResult;
import mx.babel.bansefi.banksystem.contabilidad.webservices.altaapuntemanualcuenta.ResponseBansefi;
import mx.babel.bansefi.banksystem.contabilidad.webservices.altaapuntemanualcuenta.Ejecutar.ITRAMINIAPNTEMANOTRN.TRAMINIAPNTEMANOEVT.AMAPNTEMANUALE;
import mx.babel.bansefi.banksystem.contabilidad.webservices.altaapuntemanualcuenta.Ejecutar.ITRAMINIAPNTEMANOTRN.TRAMINIAPNTEMANOEVT.CONINDFECHCRREV;
import mx.babel.bansefi.banksystem.contabilidad.wrappers.AltaApunteManualCuentaWrapper;
...
@Component
public class AltaApunteManualCuentaBackEnd extends BackEndBean implements
		Serializable {
	...
	@Autowired
	private AltaApunteManualCuentaWrapper altaApunteManualWrapper;

	private static final Logger LOGGER = LogManager
			.getLogger(AltaApunteManualCuentaBackEnd.class);
	...
	private Ejecutar.ITRAMINIAPNTEMANOTRN initPeticion(
		...
		try {
			apunte.setNUMSECAC(Long.parseLong(altaApunteBean.getCuenta()));
		} catch (NumberFormatException e) {
			apunte.setNUMSECAC(0L);
		}
		...
		datosEntrada.setTIPOOPRCN(altaApunteBean.getClaveOperacion()
				.getClaveFila());
	...
	private EjecutarResult ejecutarWS(Ejecutar.ITRAMINIAPNTEMANOTRN contexto) {
		...
		try {
			respuesta = (EjecutarResult) this.servicioWebUtils.ejecutarWS(
					AltaApunteManualCuentaServicio.class, contexto);
