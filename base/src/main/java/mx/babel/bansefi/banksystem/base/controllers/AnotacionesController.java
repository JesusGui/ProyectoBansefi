 ...
 */
@ManagedBean(name = "anotacionesController")
@ViewScoped
@Component
@Scope("view")
public class AnotacionesController{
	
	private static final Logger LOGGER = LogManager.getLogger(AnotacionesController.class.getName());
			...
			if (!this.anotaciones.get(this.posicionPersonaCuenta).getIdentificadores().contains((Integer.toString((Integer)obtieneFlash().get(ParametrosFlashEnum.ID_INTERNA.getParamFlash()))))){
				...
				if (obtieneFlash().get(ParametrosFlashEnum.NOMBRE_COMPLETO_PERSONA.getParamFlash()) != null){
					this.anotaciones.get(this.posicionPersonaCuenta).getIdentificadoresDescripciones().add((String)obtieneFlash().get(ParametrosFlashEnum.IDENTIFICACION.getParamFlash())
							+ AnotacionesController.SEPARATOR + (String)obtieneFlash().get(ParametrosFlashEnum.NOMBRE_COMPLETO_PERSONA.getParamFlash()));
					...
					this.anotaciones.get(this.posicionPersonaCuenta).getIdentificadoresDescripciones().add((String)obtieneFlash().get(ParametrosFlashEnum.ACTA_CONSTITUTIVA.getParamFlash())
							+ AnotacionesController.SEPARATOR + (String)obtieneFlash().get(ParametrosFlashEnum.RAZON_SOCIAL.getParamFlash()));
				}
				
			}else{
				RequestContext.getCurrentInstance().execute("PF('dlgDestDuplicado').show()");
			}
		}else if (obtieneFlash().get(ParametrosFlashEnum.NUMERO_CUENTA.getParamFlash()) != null) {
			this.managedBeanStateRecover.recuperaController(this);
			if (!this.anotaciones.get(this.posicionPersonaCuenta).getIdentificadores().contains(Long.toString((Long)obtieneFlash().get(ParametrosFlashEnum.NUMERO_CUENTA.getParamFlash())))){	
				this.anotaciones.get(this.posicionPersonaCuenta).getIdentificadores().add(Long.toString((Long)obtieneFlash().get(ParametrosFlashEnum.NUMERO_CUENTA.getParamFlash())));
				this.anotaciones.get(this.posicionPersonaCuenta).getIdentificadoresDescripciones().add(Long.toString((Long)obtieneFlash().get(ParametrosFlashEnum.NUMERO_CUENTA.getParamFlash()))
						+ AnotacionesController.SEPARATOR + ((String)obtieneFlash().get(ParametrosFlashEnum.TIPO_DE_CUENTA.getParamFlash())));
			}else{
				RequestContext.getCurrentInstance().execute("PF('dlgDestDuplicado').show()");
			}
		}else{
	...
	public void inicializaConsultaDetalleAnotacion() {
		...
		if (this.anotacion.getIdEmpleado().equals(AnotacionesController.BATCH_ID)){
			...
			if (anotacion.getDirigidoA().equals(ConstantesFuncionales.DEST_ANTCN.get(0))){				
				...
				if (cuenta != null && cuenta.getNumeroCuenta() !=0){
					...
					if(!existeValor(anotacion, identificador)){
						...
						anotacion.getIdentificadoresDescripciones().add(anotacion.getIdentificadorBusqueda()+AnotacionesController.SEPARATOR+
					...
					if (!clientes.isEmpty()) {
						...
						if(!existeValor(anotacion, identificador)){
							...
							anotacion.getIdentificadoresDescripciones().add(anotacion.getIdentificadorBusqueda()+
									AnotacionesController.SEPARATOR+clientes.get(0).getNombreCompleto());
