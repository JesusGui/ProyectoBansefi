...
import mx.babel.bansefi.banksystem.base.webservices.modificabienmateriales.Ejecutar;
import mx.babel.bansefi.banksystem.base.webservices.modificabienmateriales.Ejecutar.ITRMODIBIENMATERIALES;
import mx.babel.bansefi.banksystem.base.webservices.modificabienmateriales.Ejecutar.ITRMODIBIENMATERIALES.TRMODIBIENMATERIALESE.DATOSCARACTV;
import mx.babel.bansefi.banksystem.base.webservices.modificabienmateriales.EjecutarResult;
import mx.babel.bansefi.banksystem.base.webservices.modificabienmateriales.ModificaBienMaterialesServicio;
import mx.babel.bansefi.banksystem.base.webservices.modificabienmateriales.ResponseBansefi;
 ...
 */
@Component
public class ModificaBienMaterialesBackEnd extends BackEndBean {

    private static final long serialVersionUID = -8641490425636362618L;
    
    //Necesaria para que no explote TCB
    private static final String CONSTANTE_ESPACIOS = "     ";
    @Autowired
    ServicioWebUtils servicioWebUtils;

    private static final Logger LOGGER = LogManager
            .getLogger(ModificaBienMaterialesBackEnd.class.getName());

    public int ejectuarTRN(final BienBean bien) {

        LOGGER.debug("Entra al metrodo ejectuarTRN");

        final ResponseBansefi response = this.getModificaBienMaterialesResponse(bien);

        LOGGER.debug("Sale del metrodo ejectuarTRN");

        return response.getOTRMODIBIENMATERIALES().getRTRNCD();
    }

    /**
     * Función para dar de alta los datos materiales de un bien invocando un
     * servicio web.
     *
     * @param bien
     ...
     */
    public ResponseBansefi getModificaBienMaterialesResponse(final BienBean bien)
            throws NoControlableException, ControlableException {
        LOGGER.debug("Entra al metrodo getModificaBienMaterialesResponse");

        ResponseBansefi response = null;

        final Ejecutar.ITRMODIBIENMATERIALES contexto = this.initPeticion(bien);
        ...
        LOGGER.debug("Sale del metrodo getModificaBienMaterialesResponse");
        return response;
    }
     ...
     */
    private EjecutarResult ejecutarWS(final Ejecutar.ITRMODIBIENMATERIALES contexto)
            throws NoControlableException {
        EjecutarResult respuesta = null;

        try {
            respuesta = (EjecutarResult) servicioWebUtils.ejecutarWS(
                    ModificaBienMaterialesServicio.class, contexto);
        } catch (final NoControlableException e) {
            throw new NoControlableException(
                    "Error al invocar servicio web modificacion de bien materiales ",
                    e);
        }
        return respuesta;
    }
     ...
     */
    private ITRMODIBIENMATERIALES initPeticion(final BienBean bien) {

        LOGGER.debug("Entra al metrodo initPeticion");

        final DateFormat df = new SimpleDateFormat(
                ConstantesFuncionales.GENERAL_DATE_FORMATTER);

        final Ejecutar.ITRMODIBIENMATERIALES contexto = new Ejecutar.ITRMODIBIENMATERIALES();
        final Ejecutar.ITRMODIBIENMATERIALES.STDTRNIPARMV stdtrniparmv = new Ejecutar.ITRMODIBIENMATERIALES.STDTRNIPARMV();
        final Ejecutar.ITRMODIBIENMATERIALES.TRMODIBIENMATERIALESE trconsultabienmaterialese = new Ejecutar.ITRMODIBIENMATERIALES.TRMODIBIENMATERIALESE();

        stdtrniparmv.setIDINTERNOTERMTN(super.getTerminal());
        stdtrniparmv
                .setCODTX(CodTxConstants.COD_TX_TR_MODI_BIEN_MATERIALES_TRN);

        // Superficie
        if (StringUtils.isNotBlank(bien.getDatosTerreno()
                .getSuperficieConstruidaString())) {
            final DATOSCARACTV superficie = new DATOSCARACTV();
            superficie.setCODBIEN(bien.getTipoCodigo());
            superficie
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(0));
            superficie.setCODNRBEEN(super.getEntidad());
            superficie.setVALORCRCTBI(bien.getDatosTerreno()
                    .getSuperficieConstruidaString());
            if (StringUtils.isNotBlank(bien.getDatosTerreno()
                    .getSuperficieConstruidaString())) {
                superficie.setVALORCRCTBILEN(bien.getDatosTerreno()
                        .getSuperficieConstruidaString().length());
            }
            superficie.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(superficie);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosTerreno().getPartida())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosTerreno().getPartida())) {
                partida.setVALORCRCTBILEN(bien.getDatosTerreno().getPartida()
                        .length());
            }
            partida.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(partida);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosTerreno().getTipoCultivo())) {
            ...
            tipoCultivo.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(2));
            tipoCultivo.setCODNRBEEN(super.getEntidad());
            tipoCultivo.setVALORCRCTBI(bien.getDatosTerreno().getTipoCultivo());
            if (StringUtils.isNotBlank(bien.getDatosTerreno().getTipoCultivo())) {
                tipoCultivo.setVALORCRCTBILEN(bien.getDatosTerreno()
                        .getTipoCultivo().length());
            }
            tipoCultivo.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipoCultivo);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosTerreno().getTipoTerreno())) {
            ...
            tipoTerreno.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(3));
            tipoTerreno.setCODNRBEEN(super.getEntidad());
            tipoTerreno.setVALORCRCTBI(bien.getDatosTerreno().getTipoTerreno());
            if (StringUtils.isNotBlank(bien.getDatosTerreno().getTipoTerreno())) {
                tipoTerreno.setVALORCRCTBILEN(bien.getDatosTerreno()
                        .getTipoTerreno().length());
            }
            tipoTerreno.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipoTerreno);
        }
        ...
        if (bien.getDatosTerreno().getHorasRegadioString() != null) {
            ...
            horasRegadio.setVALORCRCTBI(bien.getDatosTerreno()
                    .getHorasRegadioString());
            if (bien.getDatosTerreno().getHorasRegadioString() != null) {
                horasRegadio.setVALORCRCTBILEN(
                        bien.getDatosTerreno().getHorasRegadioString().length());
            }
            horasRegadio.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(horasRegadio);
        }else{
            final DATOSCARACTV horasRegadio = new DATOSCARACTV();
            horasRegadio.setCODBIEN(bien.getTipoCodigo());
            horasRegadio.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(4));
            horasRegadio.setCODNRBEEN(super.getEntidad());
            horasRegadio.setVALORCRCTBI(ModificaBienMaterialesBackEnd.CONSTANTE_ESPACIOS);
            
            horasRegadio.setVALORCRCTBILEN(5);
        ...
        if (bien.getDatosTerreno().getOtrasString() != null) {
            ...
            horasRegadioNoPrpdad
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(5));
            horasRegadioNoPrpdad.setCODNRBEEN(super.getEntidad());
            horasRegadioNoPrpdad.setVALORCRCTBI(bien
                    .getDatosTerreno().getOtrasString());
            if (bien.getDatosTerreno().getOtrasString() != null) {
                horasRegadioNoPrpdad.setVALORCRCTBILEN(
                        bien.getDatosTerreno().getOtrasString().length());
            }
            horasRegadioNoPrpdad.setIDINTERNOBI(Integer.valueOf(bien
                    .getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(
                    horasRegadioNoPrpdad);
        }else{
            final DATOSCARACTV horasRegadioNoPrpdad = new DATOSCARACTV();
            horasRegadioNoPrpdad.setCODBIEN(bien.getTipoCodigo());
            horasRegadioNoPrpdad
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(5));
            horasRegadioNoPrpdad.setCODNRBEEN(super.getEntidad());
            horasRegadioNoPrpdad.setVALORCRCTBI(ModificaBienMaterialesBackEnd.CONSTANTE_ESPACIOS);
            
            horasRegadioNoPrpdad.setVALORCRCTBILEN(5);
        ...
        if (StringUtils.isNotBlank(bien.getDatosTerreno().getNombreFinca())) {
            ...
            nombreFinca.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(6));
            nombreFinca.setCODNRBEEN(super.getEntidad());
            nombreFinca.setVALORCRCTBI(bien.getDatosTerreno().getNombreFinca());
            if (StringUtils.isNotBlank(bien.getDatosTerreno().getNombreFinca())) {
                nombreFinca.setVALORCRCTBILEN(bien.getDatosTerreno()
                        .getNombreFinca().length());
            }
            nombreFinca.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(nombreFinca);
        }
        ...
        // trconsultabienmaterialese.getDATOSCARACTV().add(entidadValuadora);
        ...
        if (StringUtils.isNotBlank(bien.getDatosTerreno().getCarga())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosTerreno().getCarga())) {
                tipoCarga.setVALORCRCTBILEN(bien.getDatosTerreno().getCarga()
                        .length());
            }
            tipoCarga.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipoCarga);
        }
        ...
        if (bien.getDatosValuacion().getCantidad() != null) {
            ...
            cantidad.setVALORCRCTBI(String.valueOf(bien.getDatosValuacion()
                    .getCantidad()));
            if (bien.getDatosValuacion().getCantidad() != null) {
                cantidad.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosValuacion().getCantidad()).length());
            }
            cantidad.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(cantidad);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getMatricula())) {
            ...
            matricula
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(10));
            matricula.setCODNRBEEN(super.getEntidad());
            matricula.setVALORCRCTBI(bien.getDatosGenerales().getMatricula());
            if (StringUtils.isNotBlank(bien.getDatosGenerales().getMatricula())) {
                matricula.setVALORCRCTBILEN(bien.getDatosGenerales()
                        .getMatricula().length());
            }
            matricula.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(matricula);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getMarca())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosGenerales().getMarca())) {
                marca.setVALORCRCTBILEN(bien.getDatosGenerales().getMarca()
                        .length());
            }
            marca.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(marca);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getModelo())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosGenerales().getModelo())) {
                modelo.setVALORCRCTBILEN(bien.getDatosGenerales().getModelo()
                        .length());
            }
            modelo.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(modelo);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getNumeroSerie())) {
            ...
            numeroSerie.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(13));
            numeroSerie.setCODNRBEEN(super.getEntidad());
            numeroSerie.setVALORCRCTBI(bien.getDatosGenerales()
                    .getNumeroSerie());
            if (StringUtils.isNotBlank(bien.getDatosGenerales()
                    .getNumeroSerie())) {
                numeroSerie.setVALORCRCTBILEN(bien.getDatosGenerales()
                        .getNumeroSerie().length());
            }
            numeroSerie.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(numeroSerie);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getFabricante())) {
            ...
            fabricante.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(14));
            fabricante.setCODNRBEEN(super.getEntidad());
            fabricante.setVALORCRCTBI(bien.getDatosGenerales().getFabricante());
            if (StringUtils
                    .isNotBlank(bien.getDatosGenerales().getFabricante())) {
                fabricante.setVALORCRCTBILEN(bien.getDatosGenerales()
                        .getFabricante().length());
            }
            fabricante.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(fabricante);
        }

        // INVERNADERO
        if (StringUtils.isNotBlank(bien.getDatosTerreno()
                .getSuperficieInvernaderoString())) {
            final DATOSCARACTV invernadero = new DATOSCARACTV();
            invernadero.setCODBIEN(bien.getTipoCodigo());
            invernadero.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(15));
            invernadero.setCODNRBEEN(super.getEntidad());
            invernadero.setVALORCRCTBI(bien.getDatosTerreno()
                    .getSuperficieInvernaderoString());
            if (StringUtils.isNotBlank(bien.getDatosTerreno()
                    .getSuperficieInvernaderoString())) {
                invernadero.setVALORCRCTBILEN(bien.getDatosTerreno()
                        .getSuperficieInvernaderoString().length());
            }
            invernadero.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(invernadero);
        }

        // LICENCIA_DE_PESCA
        if (StringUtils
                .isNotBlank(bien.getDatosGenerales().getNumeroLicPesca())) {
            final DATOSCARACTV licenciaPesca = new DATOSCARACTV();
            licenciaPesca.setCODBIEN(bien.getTipoCodigo());
            licenciaPesca.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(16));
            licenciaPesca.setCODNRBEEN(super.getEntidad());
            licenciaPesca.setVALORCRCTBI(bien.getDatosGenerales()
                    .getNumeroLicPesca());
            if (StringUtils.isNotBlank(bien.getDatosGenerales()
                    .getNumeroLicPesca())) {
                licenciaPesca.setVALORCRCTBILEN(bien.getDatosGenerales()
                        .getNumeroLicPesca().length());
            }
            licenciaPesca.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(licenciaPesca);
        }
        ...
        if (bien.getDatosGenerales().getTonelaje() != null) {
            ...
            tonelaje.setVALORCRCTBI(String.valueOf(bien.getDatosGenerales()
                    .getTonelaje()));
            if (bien.getDatosGenerales().getTonelaje() != null) {
                tonelaje.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosGenerales().getTonelaje()).length());
            }
            tonelaje.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tonelaje);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getPuerto())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosGenerales().getPuerto())) {
                puerto.setVALORCRCTBILEN(bien.getDatosGenerales().getPuerto()
                        .length());
            }
            puerto.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(puerto);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosValuacion().getClase())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosValuacion().getClase())) {
                tipo.setVALORCRCTBILEN(bien.getDatosValuacion().getClase()
                        .length());
            }
            tipo.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipo);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosDeuda().getEntidad())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosDeuda().getEntidad())) {
                entidad.setVALORCRCTBILEN(bien.getDatosDeuda().getEntidad()
                        .length());
            }
            entidad.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(entidad);
        }
        ...
        if (bien.getDatosDeuda().getConcedidoString() != null) {
            ...
            capitalConcedido.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(23));
            capitalConcedido.setCODNRBEEN(super.getEntidad());
            capitalConcedido.setVALORCRCTBI(bien.getDatosDeuda()
                    .getConcedidoString());
            if (bien.getDatosDeuda().getConcedidoString() != null) {
                capitalConcedido.setVALORCRCTBILEN(
                        bien.getDatosDeuda().getConcedidoString().length());
            }
            capitalConcedido
                    .setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(capitalConcedido);
        }
        ...
        if (bien.getDatosDeuda().getTipoInteresString() != null) {
            ...
            tipoInteres.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(24));
            tipoInteres.setCODNRBEEN(super.getEntidad());
            tipoInteres.setVALORCRCTBI(bien.getDatosDeuda()
                    .getTipoInteresString());
            if (bien.getDatosDeuda().getTipoInteresString() != null) {
                tipoInteres.setVALORCRCTBILEN(
                        bien.getDatosDeuda().getTipoInteresString().length());
            }
            tipoInteres.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipoInteres);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosDeuda().getDuracion())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosDeuda().getDuracion())) {
                duracion.setVALORCRCTBILEN(bien.getDatosDeuda().getDuracion()
                        .length());
            }
            duracion.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(duracion);
        }

        // PERIODO_AMORTIZA
        if (StringUtils.isNotBlank(bien.getDatosDeuda()
                .getFrecuenciaAmortizacion())) {
            final DATOSCARACTV periodoAmortizacion = new DATOSCARACTV();
            periodoAmortizacion.setCODBIEN(bien.getTipoCodigo());
            periodoAmortizacion
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(26));
            periodoAmortizacion.setCODNRBEEN(super.getEntidad());
            periodoAmortizacion.setVALORCRCTBI(bien.getDatosDeuda()
                    .getFrecuenciaAmortizacion());
            if (StringUtils.isNotBlank(bien.getDatosDeuda()
                    .getFrecuenciaAmortizacion())) {
                periodoAmortizacion.setVALORCRCTBILEN(bien.getDatosDeuda()
                        .getFrecuenciaAmortizacion().length());
            }
            periodoAmortizacion.setIDINTERNOBI(Integer.valueOf(bien
                    .getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV()
                    .add(periodoAmortizacion);
        }
        ...
        if (bien.getDatosDeuda().getFechaVencimiento() != null) {
            ...
            fechaVencimiento.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(27));
            fechaVencimiento.setCODNRBEEN(super.getEntidad());
            fechaVencimiento.setVALORCRCTBI(df.format(bien.getDatosDeuda()
                    .getFechaVencimiento()));
            fechaVencimiento.setVALORCRCTBILEN(String.valueOf(
                    df.format(bien.getDatosDeuda().getFechaVencimiento()))
                    .length());

            fechaVencimiento
                    .setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(fechaVencimiento);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosDeuda().getTipoDeuda())) {
            ...
            tipoDeuda
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(28));
            tipoDeuda.setCODNRBEEN(super.getEntidad());
            tipoDeuda.setVALORCRCTBI(bien.getDatosDeuda().getTipoDeuda());
            if (StringUtils.isNotBlank(bien.getDatosDeuda().getTipoDeuda())) {
                tipoDeuda.setVALORCRCTBILEN(bien.getDatosDeuda().getTipoDeuda()
                        .length());
            }
            tipoDeuda.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipoDeuda);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosGenerales().getEstado())) {
            ...
            if (StringUtils.isNotBlank(bien.getDatosGenerales().getEstado())) {
                estado.setVALORCRCTBILEN(bien.getDatosGenerales().getEstado()
                        .length());
            }
            estado.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(estado);
        }
        ...
        if (bien.getDatosTerreno().getPrimeraVivienda() != null) {
            ...
            primeraVivienda.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(30));
            ...
            primeraVivienda
                    .setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(primeraVivienda);
        }

        // SUPERFICIE_TERRENO
        if (StringUtils.isNotBlank(bien.getDatosTerreno()
                .getSuperficieTerrenoString())) {
            final DATOSCARACTV superficieTerreno = new DATOSCARACTV();
            superficieTerreno.setCODBIEN(bien.getTipoCodigo());
            superficieTerreno.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(32));
            superficieTerreno.setCODNRBEEN(super.getEntidad());
            superficieTerreno.setVALORCRCTBI(bien.getDatosTerreno()
                    .getSuperficieTerrenoString());
            if (StringUtils.isNotBlank(bien.getDatosTerreno()
                    .getSuperficieTerrenoString())) {
                superficieTerreno.setVALORCRCTBILEN(bien.getDatosTerreno()
                        .getSuperficieTerrenoString().length());
            }
            superficieTerreno.setIDINTERNOBI(Integer.valueOf(bien
                    .getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(superficieTerreno);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosSeguro().getCiaAseguradora())) {
            ...
            ciaAseguradora.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(33));
            ciaAseguradora.setCODNRBEEN(super.getEntidad());
            ciaAseguradora.setVALORCRCTBI(bien.getDatosSeguro()
                    .getCiaAseguradora());
            if (StringUtils.isNotBlank(bien.getDatosSeguro()
                    .getCiaAseguradora())) {
                ciaAseguradora.setVALORCRCTBILEN(bien.getDatosSeguro()
                        .getCiaAseguradora().length());
            }
            ciaAseguradora.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(ciaAseguradora);
        }
        ...
        if (bien.getDatosTerreno().getPrimeraHipoteca() != null) {
            ...
            primeraHipoteca.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(34));
            ...
            primeraHipoteca
                    .setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(primeraHipoteca);
        }
        ...
        if (StringUtils.isNotBlank(bien.getDatosSeguro().getContingencia())) {
            ...
            contingencia.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(35));
            contingencia.setCODNRBEEN(super.getEntidad());
            contingencia
                    .setVALORCRCTBI(bien.getDatosSeguro().getContingencia());
            if (StringUtils.isNotBlank(bien.getDatosSeguro().getContingencia())) {
                contingencia.setVALORCRCTBILEN(bien.getDatosSeguro()
                        .getContingencia().length());
            }
            contingencia.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(contingencia);
        }
        ...
        if (bien.getDatosSeguro().getCoberturaString() != null) {
            ...
            cobertura
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(36));
            cobertura.setCODNRBEEN(super.getEntidad());
            cobertura.setVALORCRCTBI(bien.getDatosSeguro()
                    .getCoberturaString());
            if (bien.getDatosSeguro().getCoberturaString() != null) {
                cobertura.setVALORCRCTBILEN(
                        bien.getDatosSeguro().getCoberturaString().length());
            }
            cobertura.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(cobertura);
        }
        ...
        if (bien.getDatosSeguro().getBeneficiario() != null) {
            ...
            beneficiario.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(37));
            beneficiario.setCODNRBEEN(super.getEntidad());
            beneficiario.setVALORCRCTBI(String.valueOf(bien.getDatosSeguro()
                    .getBeneficiario()));
            if (bien.getDatosSeguro().getBeneficiario() != null) {
                beneficiario.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosSeguro().getBeneficiario()).length());
            }
            beneficiario.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(beneficiario);
        }
        ...
        if (bien.getDatosDeuda().getNegocioString() != null) {
            ...
            porcentajeNegocio.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(38));
            porcentajeNegocio.setCODNRBEEN(super.getEntidad());
            porcentajeNegocio.setVALORCRCTBI(bien.getDatosDeuda().getNegocioString());
            if (bien.getDatosDeuda().getNegocioString() != null) {
                porcentajeNegocio.setVALORCRCTBILEN(
                        bien.getDatosDeuda().getNegocioString().length());
            }
            porcentajeNegocio.setIDINTERNOBI(Integer.valueOf(bien
                    .getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(porcentajeNegocio);
        }

        // CIF_TASADORA
        if (StringUtils.isNotBlank(bien.getDatosValuacion()
                .getEntidadValuadora())) {
            final DATOSCARACTV cifValuadora = new DATOSCARACTV();
            cifValuadora.setCODBIEN(bien.getTipoCodigo());
            cifValuadora.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(39));
            cifValuadora.setCODNRBEEN(super.getEntidad());
            cifValuadora.setVALORCRCTBI(String.valueOf(bien.getDatosValuacion()
                    .getEntidadValuadora()));
            if (StringUtils.isNotBlank(bien.getDatosValuacion()
                    .getEntidadValuadora())) {
                cifValuadora.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosValuacion().getEntidadValuadora())
                        .length());
            }
            cifValuadora.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(cifValuadora);
        }
        ...
        if (bien.getDatosSeguro().getTipoSeguro() != null) {
            ...
            tipoSeguro.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(40));
            tipoSeguro.setCODNRBEEN(super.getEntidad());
            tipoSeguro.setVALORCRCTBI(String.valueOf(bien.getDatosSeguro()
                    .getTipoSeguro()));
            if (bien.getDatosSeguro().getTipoSeguro() != null) {
                tipoSeguro.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosSeguro().getTipoSeguro()).length());
            }
            tipoSeguro.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(tipoSeguro);
        }
        ...
        if (bien.getDatosSeguro().getPrimaAnualString() != null) {
            ...
            primaAnual.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(41));
            primaAnual.setCODNRBEEN(super.getEntidad());
            primaAnual.setVALORCRCTBI(bien.getDatosSeguro()
                    .getPrimaAnualString());
            if (bien.getDatosSeguro().getPrimaAnualString() != null) {
                primaAnual.setVALORCRCTBILEN(
                        bien.getDatosSeguro().getPrimaAnualString().length());
            }
            primaAnual.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(primaAnual);
        }
        ...
        if (bien.getDatosSeguro().getCif() != null) {
            ...
            cifSeguro
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(42));
            cifSeguro.setCODNRBEEN(super.getEntidad());
            cifSeguro.setVALORCRCTBI(String.valueOf(bien.getDatosSeguro()
                    .getCif()));
            if (bien.getDatosSeguro().getCif() != null) {
                cifSeguro.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosSeguro().getCif()).length());
            }
            cifSeguro.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(cifSeguro);
        }
        ...
        if (bien.getDatosSeguro().getFechaAntiguedad() != null) {
            ...
            fechaAntiguedad.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(43));
            fechaAntiguedad.setCODNRBEEN(super.getEntidad());
            fechaAntiguedad.setVALORCRCTBI(df.format(bien.getDatosSeguro()
                    .getFechaAntiguedad()));
            fechaAntiguedad.setVALORCRCTBILEN(String.valueOf(
                    df.format(bien.getDatosSeguro().getFechaAntiguedad()))
                    .length());

            fechaAntiguedad
                    .setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(fechaAntiguedad);
        }
        ...
        if (bien.getDatosSeguro().getFechaInicioCobertura() != null) {
            ...
            fechaInicioCobertura
                    .setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT.get(44));
            fechaInicioCobertura.setCODNRBEEN(super.getEntidad());
            fechaInicioCobertura.setVALORCRCTBI(df.format(bien.getDatosSeguro()
                    .getFechaInicioCobertura()));
            fechaInicioCobertura.setVALORCRCTBILEN(String.valueOf(
                    df.format(bien.getDatosSeguro().getFechaInicioCobertura()))
                    .length());
            fechaInicioCobertura.setIDINTERNOBI(Integer.valueOf(bien
                    .getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(
                    fechaInicioCobertura);
        }
        ...
        if (bien.getDatosSeguro().getFechaFinCobertura() != null) {
            ...
            fechaFinCobertura.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(45));
            fechaFinCobertura.setCODNRBEEN(super.getEntidad());
            fechaFinCobertura.setVALORCRCTBI(df.format(bien.getDatosSeguro()
                    .getFechaFinCobertura()));
            fechaFinCobertura.setVALORCRCTBILEN(String.valueOf(
                    df.format(bien.getDatosSeguro().getFechaFinCobertura()))
                    .length());
            fechaFinCobertura.setIDINTERNOBI(Integer.valueOf(bien
                    .getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(fechaFinCobertura);
        }
        ...
        if (bien.getDatosSeguro().getNumeroPoliza() != null) {
            ...
            numeroPoliza.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(46));
            numeroPoliza.setCODNRBEEN(super.getEntidad());
            numeroPoliza.setVALORCRCTBI(String.valueOf(bien.getDatosSeguro()
                    .getNumeroPoliza()));
            if (bien.getDatosSeguro().getNumeroPoliza() != null) {
                numeroPoliza.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosSeguro().getNumeroPoliza()).length());
            }
            numeroPoliza.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(numeroPoliza);
        }
        ...
        if (bien.getDatosSeguro().getNumeroSeguro() != null) {
            ...
            numeroSeguro.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(47));
            numeroSeguro.setCODNRBEEN(super.getEntidad());
            numeroSeguro.setVALORCRCTBI(String.valueOf(bien.getDatosSeguro()
                    .getNumeroSeguro()));
            if (bien.getDatosSeguro().getNumeroSeguro() != null) {
                numeroSeguro.setVALORCRCTBILEN(String.valueOf(
                        bien.getDatosSeguro().getNumeroSeguro()).length());
            }
            numeroSeguro.setIDINTERNOBI(Integer.valueOf(bien.getIdInterno()));
            trconsultabienmaterialese.getDATOSCARACTV().add(numeroSeguro);
        }
        ...
        if (bien.getDatosGenerales().getVerificado() != null) {
            ...
            verificado.setCODCRCTBIEN(ConstantesFuncionales.CODIGOS_CRCT
                    .get(48));
            ...
            trconsultabienmaterialese.getDATOSCARACTV().add(verificado);
        }

        contexto.setSTDTRNIPARMV(stdtrniparmv);
        contexto.setTRMODIBIENMATERIALESE(trconsultabienmaterialese);
