...
public class RetiroController implements Serializable{
	...
	public boolean comprobarInternvenientesCuenta() {
		...
		if (relacionado.get(0).getPersona().getNumIdentificacion().equals(this.retiro.getNumeroIdentificacionClienteRetiro()) && 
				ConstantesFuncionales.getRelacionIdentificacionDocumento().get(relacionado.get(0).getPersona().getTipoIdentificacion()).equals(this.retiro.getIdentificacionClienteRetiro())){
			return true;
		}else{
			return false;
		}
	...
	public boolean realizarRetiro() {
		...
		switch(retiroRes.getCodigo().trim()){
			case "ARQE159" : 	this.cuentaBloqueada = true;
								return false;
			case "ARQE135" :	this.cuentaIncorrecta = true;
								return false;
			case "DOME0004" :	this.cuentaInactiva = true;
								return false;
			case "CHEE021" : 	this.cuentaNoVista = true;
								return false;
			case "ARQE167" : 	this.saldoError = true;
								return false;
			default: 			plaza = retiroRes.getPlaza().replaceAll("\\*", "").trim();
								digito = retiroRes.getDigito();
								terminal = retiroRes.getTerminal();
								movimiento = retiroRes.getMovimiento();
								fecha = retiroRes.getFechaoprcn();
								hora = retiroRes.getHoraoprcn();
								sucursal = retiroRes.getCentro();
								String status = retiroRes.getEstatus();
								if ("0".equals(status)) {
									return true;
								} else {
									return false;
