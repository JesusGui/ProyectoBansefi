...
/* Definicion del widget del nuevo componente CalendarMonthRange */
PrimeFaces.widget.CalendarMonthRange = PrimeFaces.widget.Calendar;
/* jQuery UI - plugins sources. Redifinicion para adaptarlo a los nuevos componentes*/
	...
	if (typeof define === "function" && define.amd) {
		...
		(function(y) {
			...
			y.fn
					.extend({
						scrollParent : function() {
							var N = this.css("position"), M = N === "absolute", O = this
									.parents()
									.filter(
											function() {
												var P = y(this);
												...
												return (/(auto|scroll)/).test(P
														.css("overflow")
														+ P.css("overflow-y")
														+ P.css("overflow-x"))
											}).eq(0);
							return N === "fixed" || !O.length ? y(this[0].ownerDocument
									|| document)
									: O
						},
						uniqueId : (function() {
			...
			function n(O, M) {
				...
				return (/input|select|textarea|button|object/.test(R) ? !O.disabled
						: "a" === R ? O.href || M : M)
						&& o(O)
			}
			function o(M) {
				return y.expr.filters.visible(M)
						&& !y(M).parents().addBack().filter(function() {
			...
			y.extend(y.expr[":"], {
				...
				tabbable : function(O) {
					var M = y.attr(O, "tabindex"), N = isNaN(M);
					return (N || M >= 0) && n(O, !N)
				}
			});
			if (!y("<a>").outerWidth(1).jquery) {
				y.each([ "Width", "Height" ], function(O, M) {
					var N = M === "Width" ? [ "Left", "Right" ] : [ "Top",
							"Bottom" ], P = M.toLowerCase(), R = {
					...
					function Q(U, T, S, V) {
						y.each(N, function() {
							...
							if (S) {
								T -= parseFloat(y.css(U, "border" + this
										+ "Width")) || 0
							}
							if (V) {
			...
			if (!y.fn.addBack) {
				y.fn.addBack = function(M) {
					return this.add(M == null ? this.prevObject
							: this.prevObject.filter(M))
				}
			}
			...
			y.fn
					.extend({
						...
						disableSelection : (function() {
							var M = "onselectstart" in document
									.createElement("div") ? "selectstart"
									: "mousedown";
							return function() {
								return this.bind(M + ".ui-disableSelection",
										function(N) {
											N.preventDefault()
										})
							}
			...
			y.ui.plugin = {
				...
				call : function(M, P, O, N) {
					...
					if (!N
							&& (!M.element[0].parentNode || M.element[0].parentNode.nodeType === 11)) {
						return
					}
			 ...
			 */
			var C = 0, h = Array.prototype.slice;
					...
					for (O in N[Q]) {
						...
						if (N[Q].hasOwnProperty(O) && P !== undefined) {
							if (y.isPlainObject(P)) {
								R[O] = y.isPlainObject(R[O]) ? y.widget.extend(
										{}, R[O], P) : y.widget.extend({}, P)
							} else {
								R[O] = P
							}
						}
			...
			y.widget.bridge = function(N, M) {
				...
				y.fn[N] = function(R) {
					var P = typeof R === "string", Q = h.call(arguments, 1), S = this;
					R = !P && Q.length ? y.widget.extend.apply(null, [ R ]
							.concat(Q)) : R;
					if (P) {
						this
								.each(function() {
									...
									if (!y.isFunction(T[R])
											|| R.charAt(0) === "_") {
										return y.error("no such method '" + R
												+ "' for " + N
												+ " widget instance")
									}
									U = T[R].apply(T, Q);
			...
			y.Widget.prototype = {
				...
				_createWidget : function(M, N) {
					...
					this.options = y.widget.extend({}, this.options, this
							._getCreateOptions(), M);
					this.bindings = y();
					this.hoverable = y();
					this.focusable = y();
					if (N !== this) {
						...
						this.document = y(N.style ? N.ownerDocument
								: N.document || N);
						this.window = y(this.document[0].defaultView
				...
				destroy : function() {
					...
					this.element.unbind(this.eventNamespace).removeData(
							...
							y.camelCase(this.widgetFullName));
				...
				option : function(P, Q) {
					...
					if (typeof P === "string") {
						...
						if (R.length) {
							...
							if (arguments.length === 1) {
								return this.options[P] === undefined ? null
										: this.options[P]
							}
							M[P] = Q
						}
				...
				_setOption : function(M, N) {
					...
					if (M === "disabled") {
						this.widget().toggleClass(
								this.widgetFullName + "-disabled", !!N);
						if (N) {
							this.hoverable.removeClass("ui-state-hover");
							this.focusable.removeClass("ui-state-focus")
						}
				...
				_on : function(P, O, N) {
					...
					y.each(N, function(W, V) {
						function T() {
							if (!P
									&& (M.options.disabled === true || y(this)
											.hasClass("ui-state-disabled"))) {
								return
							}
							return (typeof V === "string" ? M[V] : V).apply(M,
									arguments)
						}
						if (typeof V !== "string") {
							T.guid = V.guid = V.guid || T.guid || y.guid++
						}
						var U = W.match(/^([\w:-]*)\s*(.*)$/), S = U[1]
								+ M.eventNamespace, R = U[2];
						if (R) {
							Q.delegate(R, S, T)
						} else {
							O.bind(S, T)
				...
				_off : function(N, M) {
					M = (M || "").split(" ").join(this.eventNamespace + " ")
							+ this.eventNamespace;
					N.unbind(M).undelegate(M)
				},
				_delay : function(P, O) {
					function N() {
						return (typeof P === "string" ? M[P] : P).apply(M,
								arguments)
					}
					var M = this;
				...
				_trigger : function(M, N, O) {
					...
					O = O || {};
					N = y.Event(N);
					N.type = (M === this.widgetEventPrefix ? M
							: this.widgetEventPrefix + M).toLowerCase();
					...
					return !(y.isFunction(P)
							&& P.apply(this.element[0], [ N ].concat(O)) === false || N
							.isDefaultPrevented())
				}
			};
			y.each({
				...
				y.Widget.prototype["_" + N] = function(Q, P, S) {
					...
					var R, O = !P ? N : P === true || typeof P === "number" ? M
							: P.effect || M;
					P = P || {};
					if (typeof P === "number") {
						P = {
					...
					.widget(
							"ui.mouse",
							{
								...
								_mouseDown : function(O) {
									...
									var N = this, P = (O.which === 1), M = (typeof this.options.cancel === "string"
											&& O.target.nodeName ? y(O.target)
									...
									if (this._mouseDistanceMet(O)
											&& this._mouseDelayMet(O)) {
									...
									if (true === y.data(O.target,
											this.widgetName
													+ ".preventClickEvent")) {
										y.removeData(O.target, this.widgetName
												+ ".preventClickEvent")
									}
								...
								_mouseMove : function(M) {
									...
									if (this._mouseDistanceMet(M)
											&& this._mouseDelayMet(M)) {
										this._mouseStarted = (this._mouseStart(
												this._mouseDownEvent, M) !== false);
										(this._mouseStarted ? this
												._mouseDrag(M) : this
												._mouseUp(M))
									}
								...
								_mouseUp : function(M) {
									...
									if (this._mouseStarted) {
										...
										if (M.target === this._mouseDownEvent.target) {
											y.data(M.target, this.widgetName
													+ ".preventClickEvent",
													true)
										}
										this._mouseStop(M)
			...
			(function() {
				...
				function Y(ac, ab, aa) {
					return [
							parseFloat(ac[0]) * (N.test(ac[0]) ? ab / 100 : 1),
							parseFloat(ac[1]) * (N.test(ac[1]) ? aa / 100 : 1) ]
				}
				function S(aa, ab) {
				...
				y.position = {
					scrollbarWidth : function() {
						...
						var ab, aa, ad = y("<div style='display:block;position:absolute;width:50px;height:50px;overflow:hidden;'><div style='height:100px;width:auto;'></div></div>"), ac = ad
								.children()[0];
						y("body").append(ad);
						ab = ac.offsetWidth;
					...
					getScrollInfo : function(ae) {
						var ad = ae.isWindow || ae.isDocument ? "" : ae.element
								.css("overflow-x"), ac = ae.isWindow
								|| ae.isDocument ? "" : ae.element
								.css("overflow-y"), ab = ad === "scroll"
								|| (ad === "auto" && ae.width < ae.element[0].scrollWidth), aa = ac === "scroll"
								|| (ac === "auto" && ae.height < ae.element[0].scrollHeight);
						return {
					...
					getWithinInfo : function(ab) {
						var ac = y(ab || window), aa = y.isWindow(ac[0]), ad = !!ac[0]
								&& ac[0].nodeType === 9;
						return {
							element : ac,
							isWindow : aa,
				...
				y.fn.position = function(ak) {
					...
					ak = y.extend({}, ak);
					var al, ah, af, aj, ae, aa, ag = y(ak.of), ad = y.position
							.getWithinInfo(ak.within), ab = y.position
							.getScrollInfo(ad), ai = (ak.collision || "flip")
							.split(" "), ac = {};
					aa = P(ag);
					...
					y.each([ "my", "at" ], function() {
						...
						if (ao.length === 1) {
							ao = O.test(ao[0]) ? ao.concat([ "center" ]) : R
									.test(ao[0]) ? [ "center" ].concat(ao) : [
									"center", "center" ]
						}
					...
					return this
							.each(function() {
								var an, ax, ap = y(this), ar = ap.outerWidth(), ao = ap
										.outerHeight(), aq = S(this,
										"marginLeft"), am = S(this, "marginTop"), aw = ar
										+ aq
										+ S(this, "marginRight")
										+ ab.width, av = ao + am
										+ S(this, "marginBottom") + ab.height, at = y
										.extend({}, ae), au = Y(ac.my, ap
										.outerWidth(), ap.outerHeight());
								if (ak.my[0] === "right") {
									at.left -= ar
								} else {
									if (ak.my[0] === "center") {
								...
								if (ak.using) {
									ax = function(aB) {
										var aD = aj.left - at.left, aA = aD
												+ ah - ar, aC = aj.top - at.top, az = aC
												+ af - ao, ay = {
											target : {
												element : ag,
												left : aj.left,
				...
				y.ui.position = {
					fit : {
						left : function(ae, ad) {
							var ac = ad.within, ag = ac.isWindow ? ac.scrollLeft
									: ac.offset.left, ai = ac.width, af = ae.left
									- ad.collisionPosition.marginLeft, ah = ag
									- af, ab = af + ad.collisionWidth - ai - ag, aa;
							if (ad.collisionWidth > ai) {
								if (ah > 0 && ab <= 0) {
									aa = ae.left + ah + ad.collisionWidth - ai
											- ag;
									ae.left += ah - aa
								} else {
									if (ab > 0 && ah <= 0) {
						...
						top : function(ad, ac) {
							var ab = ac.within, ah = ab.isWindow ? ab.scrollTop
									: ab.offset.top, ai = ac.within.height, af = ad.top
									- ac.collisionPosition.marginTop, ag = ah
									- af, ae = af + ac.collisionHeight - ai
									- ah, aa;
							if (ac.collisionHeight > ai) {
								if (ag > 0 && ae <= 0) {
									aa = ad.top + ag + ac.collisionHeight - ai
											- ah;
									ad.top += ag - aa
								} else {
									if (ae > 0 && ag <= 0) {
					...
					flip : {
						left : function(ag, af) {
							var ae = af.within, ak = ae.offset.left
									+ ae.scrollLeft, an = ae.width, ac = ae.isWindow ? ae.scrollLeft
									: ae.offset.left, ah = ag.left
									- af.collisionPosition.marginLeft, al = ah
									- ac, ab = ah + af.collisionWidth - an - ac, aj = af.my[0] === "left" ? -af.elemWidth
									: af.my[0] === "right" ? af.elemWidth : 0, am = af.at[0] === "left" ? af.targetWidth
									: af.at[0] === "right" ? -af.targetWidth
											: 0, ad = -2 * af.offset[0], aa, ai;
							if (al < 0) {
								aa = ag.left + aj + am + ad + af.collisionWidth
										- an - ak;
								if (aa < 0 || aa < Z(al)) {
									ag.left += aj + am + ad
								}
								...
								if (ab > 0) {
									ai = ag.left
											- af.collisionPosition.marginLeft
											+ aj + am + ad - ac;
									if (ai > 0 || Z(ai) < ab) {
										ag.left += aj + am + ad
									}
						...
						top : function(af, ae) {
							var ad = ae.within, am = ad.offset.top
									+ ad.scrollTop, an = ad.height, aa = ad.isWindow ? ad.scrollTop
									: ad.offset.top, ah = af.top
									- ae.collisionPosition.marginTop, aj = ah
									- aa, ag = ah + ae.collisionHeight - an
									- aa, ak = ae.my[1] === "top", ai = ak ? -ae.elemHeight
									: ae.my[1] === "bottom" ? ae.elemHeight : 0, ao = ae.at[1] === "top" ? ae.targetHeight
									: ae.at[1] === "bottom" ? -ae.targetHeight
											: 0, ac = -2 * ae.offset[1], al, ab;
							if (aj < 0) {
								ab = af.top + ai + ao + ac + ae.collisionHeight
										- an - am;
								if ((af.top + ai + ao + ac) > aj
										&& (ab < 0 || ab < Z(aj))) {
									af.top += ai + ao + ac
								}
							} else {
								if (ag > 0) {
									al = af.top
											- ae.collisionPosition.marginTop
											+ ai + ao + ac - aa;
									if ((af.top + ai + ao + ac) > ag
											&& (al > 0 || Z(al) < ag)) {
										af.top += ai + ao + ac
									}
								}
				...
				(function() {
					var ae, ag, ab, ad, ac, aa = document
							.getElementsByTagName("body")[0], af = document
							.createElement("div");
					...
					.widget(
							...
							y.ui.mouse,
							{
								...
								_mouseCapture : function(O) {
									var M = this.document[0], P = this.options;
									try {
										if (M.activeElement
								...
								_mouseStart : function(M) {
									...
									this.originalPosition = this.position = this
											._generatePosition(M, false);
									this.originalPageX = M.pageX;
									this.originalPageY = M.pageY;
									(N.cursorAt && this
											._adjustOffsetFromHelper(N.cursorAt));
								...
								_mouseDrag : function(M, O) {
									...
									this.position = this._generatePosition(M,
											true);
								...
								_mouseStop : function(N) {
									...
									if ((this.options.revert === "invalid" && !O)
											|| (this.options.revert === "valid" && O)
								...
								_mouseUp : function(M) {
									y("div.ui-draggable-iframeFix").each(
											function() {
												this.parentNode
														.removeChild(this)
											});
									...
									this.element.focus();
									return y.ui.mouse.prototype._mouseUp.call(
											this, M)
								},
								cancel : function() {
									if (this.helper
								...
								_getHandle : function(M) {
									return this.options.handle ? !!y(M.target)
											.closest(
													this.element
								...
								_createHelper : function(N) {
									var O = this.options, M = y
											.isFunction(O.helper) ? y(O.helper
											.apply(this.element[0], [ N ]))
								...
								_adjustOffsetFromHelper : function(M) {
									...
									if ("left" in M) {
										this.offset.click.left = M.left
												+ this.margins.left
									}
									if ("right" in M) {
										this.offset.click.left = this.helperProportions.width
												- M.right + this.margins.left
									}
									if ("top" in M) {
										this.offset.click.top = M.top
												+ this.margins.top
									}
									if ("bottom" in M) {
										this.offset.click.top = this.helperProportions.height
												- M.bottom + this.margins.top
									}
								},
								_isRootNode : function(M) {
									return (/(html|body)/i).test(M.tagName)
											|| M === this.document[0]
								},
								_getParentOffset : function() {
									var N = this.offsetParent.offset(), M = this.document[0];
									if (this.cssPosition === "absolute"
											&& this.scrollParent[0] !== M
											&& y.contains(this.scrollParent[0],
													this.offsetParent[0])) {
										N.left += this.scrollParent
												.scrollLeft();
								...
								_getRelativeOffset : function() {
									...
									var M = this.element.position(), N = this
								...
								_setContainment : function() {
									...
									if (P.containment === "window") {
										this.containment = [
												...
												y(window).scrollLeft()
														...
														- this.margins.left,
												y(window).scrollTop()
														+ (y(window).height() || M.body.parentNode.scrollHeight)
									...
									if (P.containment === "document") {
										this.containment = [
												...
												y(M).width()
														- this.helperProportions.width
														- this.margins.left,
												(y(M).height() || M.body.parentNode.scrollHeight)
														- this.helperProportions.height
														- this.margins.top ];
										return
									}
									...
									this.containment = [
											(parseInt(Q.css("borderLeftWidth"),
													10) || 0)
													+ (parseInt(
															Q
																	.css("paddingLeft"),
															10) || 0),
											(parseInt(Q.css("borderTopWidth"),
													10) || 0)
													+ (parseInt(Q
															.css("paddingTop"),
															10) || 0),
											(O ? Math.max(N.scrollWidth,
													N.offsetWidth)
													: N.offsetWidth)
													- (parseInt(
															Q
								...
								_convertPositionTo : function(N, P) {
									...
									var M = N === "absolute" ? 1 : -1, O = this
											._isRootNode(this.scrollParent[0]);
									return {
										top : (P.top + this.offset.relative.top
												* M + this.offset.parent.top
												* M - ((this.cssPosition === "fixed" ? -this.offset.scroll.top
												: (O ? 0
														: this.offset.scroll.top)) * M)),
										left : (P.left
												+ this.offset.relative.left * M
												+ this.offset.parent.left * M - ((this.cssPosition === "fixed" ? -this.offset.scroll.left
												: (O ? 0
														: this.offset.scroll.left)) * M))
									}
								},
								_generatePosition : function(N, T) {
									var M, U, V, P, O = this.options, S = this
											._isRootNode(this.scrollParent[0]), R = N.pageX, Q = N.pageY;
									...
									if (T) {
										...
										if (O.grid) {
											V = O.grid[1] ? this.originalPageY
													+ Math
															.round((Q - this.originalPageY)
																	/ O.grid[1])
													* O.grid[1]
													: this.originalPageY;
											Q = M ? ((V - this.offset.click.top >= M[1] || V
													- this.offset.click.top > M[3]) ? V
													: ((V
															- this.offset.click.top >= M[1]) ? V
															- O.grid[1]
															: V + O.grid[1]))
													: V;
											P = O.grid[0] ? this.originalPageX
													+ Math
															.round((R - this.originalPageX)
																	/ O.grid[0])
													* O.grid[0]
													: this.originalPageX;
											R = M ? ((P
													- this.offset.click.left >= M[0] || P
													- this.offset.click.left > M[2]) ? P
													: ((P
															- this.offset.click.left >= M[0]) ? P
															- O.grid[0]
															: P + O.grid[0]))
									...
									return {
										top : (Q - this.offset.click.top
												- this.offset.relative.top
												- this.offset.parent.top + (this.cssPosition === "fixed" ? -this.offset.scroll.top
												: (S ? 0
														: this.offset.scroll.top))),
										left : (R - this.offset.click.left
												- this.offset.relative.left
												- this.offset.parent.left + (this.cssPosition === "fixed" ? -this.offset.scroll.left
												: (S ? 0
								...
								_trigger : function(M, N, O) {
									O = O || this._uiHash();
									y.ui.plugin.call(this, M, [ N, O, this ],
											true);
									if (M === "drag") {
										this.positionAbs = this
												._convertPositionTo("absolute")
									}
									return y.Widget.prototype._trigger.call(
											this, M, N, O)
								},
								plugins : {},
								_uiHash : function() {
													...
													function() {
														...
														if (Q) {
															if (!this.instance.isOver) {
																...
																this.instance.offset.parent.left -= O.offset.parent.left
																		- this.instance.offset.parent.left;
																this.instance.offset.parent.top -= O.offset.parent.top
					...
					.add(
							...
							"scroll",
							{
								start : function(N, O, M) {
									if (M.scrollParent[0] !== M.document[0]
											&& M.scrollParent[0].tagName !== "HTML") {
										M.overflowOffset = M.scrollParent
												.offset()
									}
								},
								drag : function(P, Q, O) {
									var R = O.options, N = false, M = O.document[0];
									if (O.scrollParent[0] !== M
											&& O.scrollParent[0].tagName !== "HTML") {
										if (!R.axis || R.axis !== "x") {
											if ((O.overflowOffset.top + O.scrollParent[0].offsetHeight)
													- P.pageY < R.scrollSensitivity) {
												O.scrollParent[0].scrollTop = N = O.scrollParent[0].scrollTop
														+ R.scrollSpeed
											} else {
												if (P.pageY
														- O.overflowOffset.top < R.scrollSensitivity) {
													O.scrollParent[0].scrollTop = N = O.scrollParent[0].scrollTop
															- R.scrollSpeed
												}
											}
										...
										if (!R.axis || R.axis !== "y") {
											if ((O.overflowOffset.left + O.scrollParent[0].offsetWidth)
													- P.pageX < R.scrollSensitivity) {
												O.scrollParent[0].scrollLeft = N = O.scrollParent[0].scrollLeft
														+ R.scrollSpeed
											} else {
												if (P.pageX
														- O.overflowOffset.left < R.scrollSensitivity) {
													O.scrollParent[0].scrollLeft = N = O.scrollParent[0].scrollLeft
															- R.scrollSpeed
												}
											}
					...
					.add(
							...
							"snap",
							{
								start : function(N, O, M) {
									...
									M.snapElements = [];
									y(
											P.snap.constructor !== String ? (P.snap.items || ":data(ui-draggable)")
													: P.snap).each(function() {
										var R = y(this), Q = R.offset();
										if (this !== M.element[0]) {
											M.snapElements.push({
												item : this,
												width : R.outerWidth(),
								...
								drag : function(Y, V, P) {
									var M, ad, R, S, X, U, T, ae, Z, Q, W = P.options, ac = W.snapTolerance, ab = V.offset.left, aa = ab
											+ P.helperProportions.width, O = V.offset.top, N = O
									...
									for (Z = P.snapElements.length - 1; Z >= 0; Z--) {
										...
										if (aa < X - ac
												...
												|| !y
														.contains(
																P.snapElements[Z].item.ownerDocument,
																P.snapElements[Z].item)) {
											if (P.snapElements[Z].snapping) {
												(P.options.snap.release && P.options.snap.release
														.call(
																P.element,
																Y,
																y
										...
										if (!P.snapElements[Z].snapping
												&& (M || ad || R || S || Q)) {
											(P.options.snap.snap && P.options.snap.snap
													.call(
															P.element,
															Y,
															y
										...
										P.snapElements[Z].snapping = (M || ad
												|| R || S || Q)
									}
								}
							});
					...
					.widget(
							"ui.droppable",
							{
								...
								_deactivate : function(N) {
									...
									if (M) {
										this._trigger("deactivate", N, this
												.ui(M))
									}
								},
								_over : function(N) {
									var M = y.ui.ddmanager.current;
									if (!M
											|| (M.currentItem || M.element)[0] === this.element[0]) {
										return
									}
									if (this.accept.call(this.element[0],
											(M.currentItem || M.element))) {
								...
								_drop : function(N, O) {
									var M = O || y.ui.ddmanager.current, P = false;
									if (!M
											|| (M.currentItem || M.element)[0] === this.element[0]) {
										return false
									}
									this.element
											...
											.each(
													function() {
														...
														if (Q.options.greedy
																&& !Q.options.disabled
																&& Q.options.scope === M.options.scope
																&& Q.accept
																		.call(
																				Q.element[0],
																				(M.currentItem || M.element))
			...
			y.ui.intersect = (function() {
				...
				return function(Z, T, X) {
					...
					var R, S, P = (Z.positionAbs || Z.position.absolute).left, W = (Z.positionAbs || Z.position.absolute).top, O = P
							+ Z.helperProportions.width, V = W
							+ Z.helperProportions.height, Q = T.offset.left, Y = T.offset.top, N = Q
							+ T.proportions().width, U = Y
							+ T.proportions().height;
					switch (X) {
					case "fit":
						return (Q <= P && O <= N && Y <= W && V <= U);
					case "intersect":
						return (Q < P + (Z.helperProportions.width / 2)
								&& O - (Z.helperProportions.width / 2) < N
								&& Y < W + (Z.helperProportions.height / 2) && V
								- (Z.helperProportions.height / 2) < U);
					case "pointer":
						...
						return M(S, Y, T.proportions().height)
								&& M(R, Q, T.proportions().width);
					case "touch":
						return ((W >= Y && W <= U) || (V >= Y && V <= U) || (W < Y && V > U))
								&& ((P >= Q && P <= N) || (O >= Q && O <= N) || (P < Q && O > N));
					default:
						return false
					}
				}
			...
			y.ui.ddmanager = {
				...
				prepareOffsets : function(P, R) {
					var O, N, M = y.ui.ddmanager.droppables[P.options.scope]
							|| [], Q = R ? R.type : null, S = (P.currentItem || P.element)
							.find(":data(ui-droppable)").addBack();
					droppablesLoop: for (O = 0; O < M.length; O++) {
						if (M[O].options.disabled
								|| (P && !M[O].accept.call(M[O].element[0],
				...
				drop : function(M, N) {
					var O = false;
					y.each((y.ui.ddmanager.droppables[M.options.scope] || [])
							.slice(), function() {
						if (!this.options) {
							return
						}
				...
				dragStart : function(M, N) {
					M.element.parentsUntil("body").bind("scroll.droppable",
							function() {
				...
				drag : function(M, N) {
					...
					y
							.each(
									y.ui.ddmanager.droppables[M.options.scope]
											|| [],
									function() {
										if (this.options.disabled
												|| this.greedyChild
										...
										var R, P, O, Q = y.ui.intersect(M,
												this, this.options.tolerance), S = !Q
												&& this.isover ? "isout" : (Q
												&& !this.isover ? "isover"
												: null);
										if (!S) {
					...
					.widget(
							...
							y.ui.mouse,
							{
								...
								_create : function() {
									...
									if (this.handles.constructor === String) {
										...
										for (N = 0; N < S.length; N++) {
											Q = y.trim(S[N]);
											M = "ui-resizable-" + Q;
											O = y("<div class='ui-resizable-handle "
													+ M + "'></div>");
											O.css({
												zIndex : R.zIndex
											});
									...
									this._handles = y(".ui-resizable-handle",
								...
								_mouseStart : function(N) {
									...
									this.aspectRatio = (typeof P.aspectRatio === "number") ? P.aspectRatio
											: ((this.originalSize.width / this.originalSize.height) || 1);
									Q = y(".ui-resizable-" + this.axis).css(
											"cursor");
									y("body").css(
											"cursor",
											Q === "auto" ? this.axis
								...
								_mouseStop : function(P) {
									...
									if (this._helper) {
										...
										M = O.length
												&& (/textarea/i)
														.test(O[0].nodeName);
										N = M && this._hasScroll(O[0], "left") ? 0
												: T.sizeDiff.height;
										...
										R = (parseInt(T.element.css("left"), 10) + (T.position.left - T.originalPosition.left))
												|| null;
										U = (parseInt(T.element.css("top"), 10) + (T.position.top - T.originalPosition.top))
												|| null;
										if (!Q.animate) {
								...
								_updateVirtualBoundaries : function(O) {
									...
									M = {
										minWidth : this._isNumber(R.minWidth) ? R.minWidth
												: 0,
										maxWidth : this._isNumber(R.maxWidth) ? R.maxWidth
												: Infinity,
										minHeight : this._isNumber(R.minHeight) ? R.minHeight
												: 0,
										maxHeight : this._isNumber(R.maxHeight) ? R.maxHeight
												: Infinity
									};
								...
								_updateRatio : function(O) {
									var P = this.position, N = this.size, M = this.axis;
								...
								_respectSize : function(R) {
									var O = this._vBoundaries, U = this.axis, W = this
											._isNumber(R.width)
											&& O.maxWidth
											&& (O.maxWidth < R.width), S = this
											._isNumber(R.height)
											&& O.maxHeight
											&& (O.maxHeight < R.height), P = this
											._isNumber(R.width)
											&& O.minWidth
											&& (O.minWidth > R.width), V = this
											._isNumber(R.height)
											&& O.minHeight
											&& (O.minHeight > R.height), N = this.originalPosition.left
											+ this.originalSize.width, T = this.position.top
											+ this.size.height, Q = /sw|nw|w/
											.test(U), M = /nw|ne|n/.test(U);
									if (P) {
										R.width = O.minWidth
									}
									if (V) {
								...
								_proportionallyResize : function() {
									...
									var P, N, R, M, Q, O = this.helper
											|| this.element;
									for (P = 0; P < this._proportionallyResizeElements.length; P++) {
										Q = this._proportionallyResizeElements[P];
										if (!this.borderDif) {
											this.borderDif = [];
											R = [ Q.css("borderTopWidth"),
													Q.css("borderRightWidth"),
													Q.css("borderBottomWidth"),
													Q.css("borderLeftWidth") ];
											M = [ Q.css("paddingTop"),
													Q.css("paddingRight"),
													Q.css("paddingBottom"),
													Q.css("paddingLeft") ];
											for (N = 0; N < R.length; N++) {
												this.borderDif[N] = (parseInt(
														R[N], 10) || 0)
														+ (parseInt(M[N], 10) || 0)
											}
										}
								...
								_renderProxy : function() {
									var M = this.element, N = this.options;
									this.elementOffset = M.offset();
									if (this._helper) {
										this.helper = this.helper
												|| y("<div style='overflow:hidden;'></div>");
										this.helper
								...
								_change : {
									...
									w : function(O, M) {
										var N = this.originalSize, P = this.originalPosition;
										return {
											left : P.left + M,
											width : N.width - M
										}
									...
									n : function(P, N, M) {
										var O = this.originalSize, Q = this.originalPosition;
										return {
											top : Q.top + M,
											height : O.height - M
										}
									...
									se : function(O, N, M) {
										return y.extend(this._change.s.apply(
												this, arguments),
												this._change.e.apply(this, [ O,
														N, M ]))
									},
									sw : function(O, N, M) {
										return y.extend(this._change.s.apply(
												this, arguments),
												this._change.w.apply(this, [ O,
														N, M ]))
									},
									ne : function(O, N, M) {
										return y.extend(this._change.n.apply(
												this, arguments),
												this._change.e.apply(this, [ O,
														N, M ]))
									},
								...
								_propagate : function(N, M) {
									...
									(N !== "resize" && this._trigger(N, M, this
											.ui()))
								},
								plugins : {},
								ui : function() {
			...
			y.ui.plugin
					.add(
							...
							{
								stop : function(P) {
									var U = y(this).resizable("instance"), R = U.options, O = U._proportionallyResizeElements, M = O.length
											&& (/textarea/i)
													.test(O[0].nodeName), N = M
											&& U._hasScroll(O[0], "left") ? 0
											: U.sizeDiff.height, T = M ? 0
											: U.sizeDiff.width, Q = {
										width : (U.size.width - T),
										height : (U.size.height - N)
									}, S = (parseInt(U.element.css("left"), 10) + (U.position.left - U.originalPosition.left))
											|| null, V = (parseInt(U.element
											.css("top"), 10) + (U.position.top - U.originalPosition.top))
											|| null;
									U.element.animate(y.extend(Q, V && S ? {
										...
										step : function() {
											var W = {
												width : parseInt(U.element
														.css("width"), 10),
												height : parseInt(U.element
														.css("height"), 10),
												top : parseInt(U.element
														.css("top"), 10),
												left : parseInt(U.element
														.css("left"), 10)
											};
											if (O && O.length) {
												y(O[0]).css({
			...
			y.ui.plugin
					.add(
							...
							{
								start : function() {
									var U, O, W, M, T, P, X, V = y(this)
											.resizable("instance"), S = V.options, R = V.element, N = S.containment, Q = (N instanceof y) ? N
											.get(0)
											: (/parent/.test(N)) ? R.parent()
													.get(0) : N;
									if (!Q) {
										return
									}
									...
									if (/document/.test(N) || N === document) {
										...
										P = (V._hasScroll(Q, "left") ? Q.scrollWidth
												: T);
										X = (V._hasScroll(Q) ? Q.scrollHeight
												: M);
										V.parentData = {
											element : Q,
											left : W.left,
								...
								resize : function(N, X) {
									var T, Z, S, Q, U = y(this).resizable(
											"instance"), P = U.options, W = U.containerOffset, V = U.position, Y = U._aspectRatio
											|| N.shiftKey, M = {
										top : 0,
										left : 0
									...
									if (V.left < (U._helper ? W.left : 0)) {
										U.size.width = U.size.width
												+ (U._helper ? (U.position.left - W.left)
														: (U.position.left - M.left));
										if (Y) {
									...
									if (V.top < (U._helper ? W.top : 0)) {
										U.size.height = U.size.height
												+ (U._helper ? (U.position.top - W.top)
														: U.position.top);
										if (Y) {
									...
									U.offset.left = U.parentData.left
											+ U.position.left;
									U.offset.top = U.parentData.top
											+ U.position.top;
									T = Math
											.abs((U._helper ? U.offset.left
													- M.left
													: (U.offset.left - W.left))
													+ U.sizeDiff.width);
									Z = Math.abs((U._helper ? U.offset.top
											- M.top : (U.offset.top - W.top))
											+ U.sizeDiff.height);
									S = U.containerElement.get(0) === U.element
											.parent().get(0);
									Q = /relative|absolute/
											.test(U.containerElement
													.css("position"));
									if (S && Q) {
								...
								stop : function() {
									var R = y(this).resizable("instance"), N = R.options, S = R.containerOffset, M = R.containerPosition, O = R.containerElement, P = y(R.helper), U = P
											.offset(), T = P.outerWidth()
											- R.sizeDiff.width, Q = P
											.outerHeight()
											- R.sizeDiff.height;
									if (R._helper
											&& !N.animate
			...
			y.ui.plugin
					.add(
							...
							{
								start : function() {
									var M = y(this).resizable("instance"), O = M.options, N = function(
											P) {
										y(P)
												.each(
														function() {
								...
								resize : function(O, Q) {
									var N = y(this).resizable("instance"), R = N.options, P = N.originalSize, T = N.originalPosition, S = {
												...
												.each(
														function() {
															var Y = y(this), Z = y(
																	this)
																	.data(
																			"ui-resizable-alsoresize"), X = {}, W = V
																	&& V.length ? V
																	: Y
																			.parents(Q.originalElement[0]).length ? [
																			"width",
																			"height" ]
																			: [
																					"width",
									...
									if (typeof (R.alsoResize) === "object"
											&& !R.alsoResize.nodeType) {
			...
			y.ui.plugin
					.add(
							...
							{
								start : function() {
									var N = y(this).resizable("instance"), O = N.options, M = N.size;
			...
			y.ui.plugin
					.add(
							...
							{
								resize : function() {
									var Y = y(this).resizable("instance"), Q = Y.options, Z = Y.size, S = Y.originalSize, V = Y.originalPosition, aa = Y.axis, N = typeof Q.grid === "number" ? [
											Q.grid, Q.grid ]
											: Q.grid, W = (N[0] || 1), U = (N[1] || 1), P = Math
											.round((Z.width - S.width) / W)
											* W, O = Math
											.round((Z.height - S.height) / U)
											* U, T = S.width + P, M = S.height
											+ O, R = Q.maxWidth
											&& (Q.maxWidth < T), ab = Q.maxHeight
											&& (Q.maxHeight < M), X = Q.minWidth
											&& (Q.minWidth > T), ac = Q.minHeight
											&& (Q.minHeight > M);
									Q.grid = N;
									if (X) {
										T = T + W
										...
										if (/^(ne)$/.test(aa)) {
											...
											if (/^(sw)$/.test(aa)) {
												...
												if (M - U > 0) {
													...
													Y.size.height = U;
													Y.position.top = V.top
															+ S.height - U
												}
												if (T - W > 0) {
													Y.size.width = T;
													Y.position.left = V.left
															- P
												} else {
													Y.size.width = W;
													Y.position.left = V.left
							...
							y.ui.mouse,
							{
								...
								_mouseDrag : function(T) {
									...
									this.selectees
											.each(function() {
												var U = y.data(this,
														"selectable-item"), V = false;
												if (!U
														|| U.element === S.element[0]) {
													return
												}
								...
								_mouseStop : function(N) {
									var M = this;
									this.dragged = false;
									y(".ui-unselecting", this.element[0])
											.each(
													function() {
														var O = y
																.data(this,
					...
					.widget(
							...
							y.ui.mouse,
							{
								...
								_isFloating : function(M) {
									return (/left|right/).test(M.css("float"))
											|| (/inline|table-cell/).test(M
													.css("display"))
								},
								_create : function() {
									var M = this.options;
									...
									this.refresh();
									this.floating = this.items.length ? M.axis === "x"
											|| this
								...
								_mouseCapture : function(O, P) {
									...
									y(O.target).parents().each(
											function() {
												if (y.data(this, N.widgetName
														+ "-item") === N) {
													M = y(this);
													return false
												}
								...
								_mouseStart : function(P, Q, N) {
									...
									(R.cursorAt && this
											._adjustOffsetFromHelper(R.cursorAt));
									this.domPosition = {
								...
								_mouseDrag : function(Q) {
									...
									if (this.options.scroll) {
										if (this.scrollParent[0] !== document
											...
											if ((this.overflowOffset.top + this.scrollParent[0].offsetHeight)
													- Q.pageY < R.scrollSensitivity) {
												this.scrollParent[0].scrollTop = M = this.scrollParent[0].scrollTop
														+ R.scrollSpeed
											} else {
												if (Q.pageY
														- this.overflowOffset.top < R.scrollSensitivity) {
													this.scrollParent[0].scrollTop = M = this.scrollParent[0].scrollTop
															- R.scrollSpeed
												}
											}
											if ((this.overflowOffset.left + this.scrollParent[0].offsetWidth)
													- Q.pageX < R.scrollSensitivity) {
												this.scrollParent[0].scrollLeft = M = this.scrollParent[0].scrollLeft
														+ R.scrollSpeed
											} else {
												if (Q.pageX
														- this.overflowOffset.left < R.scrollSensitivity) {
													this.scrollParent[0].scrollLeft = M = this.scrollParent[0].scrollLeft
															- R.scrollSpeed
												}
											}
											...
											if (Q.pageY
													- y(document).scrollTop() < R.scrollSensitivity) {
												M = y(document)
														.scrollTop(
																y(document)
																		.scrollTop()
											...
											if (Q.pageX
													- y(document).scrollLeft() < R.scrollSensitivity) {
												M = y(document)
														.scrollLeft(
																y(document)
																		.scrollLeft()
										...
										if (M !== false && y.ui.ddmanager
												&& !R.dropBehaviour) {
											y.ui.ddmanager.prepareOffsets(this,
													Q)
										}
									}
									this.positionAbs = this
									...
									for (O = this.items.length - 1; O >= 0; O--) {
										...
										if (N !== this.currentItem[0]
												&& this.placeholder[S === 1 ? "next"
														: "prev"]()[0] !== N
												&& !y.contains(
														this.placeholder[0], N)
												&& (this.options.type === "semi-dynamic" ? !y
														.contains(
																this.element[0],
																N)
														: true)) {
											this.direction = S === 1 ? "down"
													: "up";
											if (this.options.tolerance === "pointer"
													|| this
															._intersectsWithSides(P)) {
												this._rearrange(Q, P)
											} else {
												break
											}
											this._trigger("change", Q, this
													._uiHash());
											break
										}
									}
								...
								_mouseStop : function(O, Q) {
									...
									if (this.options.revert) {
										var N = this, R = this.placeholder
												.offset(), M = this.options.axis, P = {};
										if (!M || M === "x") {
											P.left = R.left
													- this.offset.parent.left
													- this.margins.left
								...
								cancel : function() {
									...
									if (this.placeholder) {
										...
										if (this.domPosition.prev) {
											y(this.domPosition.prev).after(
													this.currentItem)
										} else {
											y(this.domPosition.parent).prepend(
													this.currentItem)
										}
									}
									return this
								...
								serialize : function(O) {
									var M = this._getItemsAsjQuery(O
											&& O.connected), N = [];
									O = O || {};
									y(M)
											.each(
								...
								_intersectsWith : function(X) {
									var O = this.positionAbs.left, N = O
											+ this.helperProportions.width, V = this.positionAbs.top, U = V
											+ this.helperProportions.height, P = X.left, M = P
											+ X.width, Y = X.top, T = Y
											+ X.height, Z = this.offset.click.top, S = this.offset.click.left, R = (this.options.axis === "x")
											|| ((V + Z) > Y && (V + Z) < T), W = (this.options.axis === "y")
											|| ((O + S) > P && (O + S) < M), Q = R
											&& W;
									if (this.options.tolerance === "pointer"
											|| this.options.forcePointerForContainers
											|| (this.options.tolerance !== "pointer" && this.helperProportions[this.floating ? "width"
													: "height"] > X[this.floating ? "width"
													: "height"])) {
										return Q
									} else {
										return (P < O
												...
												&& N
														- (this.helperProportions.width / 2) < M
												&& Y < V
														+ (this.helperProportions.height / 2) && U
												- (this.helperProportions.height / 2) < T)
									}
								},
								_intersectsWithPointer : function(O) {
									var P = (this.options.axis === "x")
											|| this
													._isOverAxis(
															this.positionAbs.top
																	+ this.offset.click.top,
															O.top, O.height), N = (this.options.axis === "y")
											|| this
													._isOverAxis(
															...
															O.left, O.width), R = P
											&& N, M = this
											._getDragVerticalDirection(), Q = this
											._getDragHorizontalDirection();
									if (!R) {
										return false
									}
									return this.floating ? (((Q && Q === "right") || M === "down") ? 2
											: 1)
											: (M && (M === "down" ? 2 : 1))
								},
								_intersectsWithSides : function(P) {
									var N = this._isOverAxis(
											this.positionAbs.top
													+ this.offset.click.top,
											P.top + (P.height / 2), P.height), O = this
								...
								_connectWith : function() {
									var M = this.options;
									return M.connectWith.constructor === String ? [ M.connectWith ]
											: M.connectWith
								},
								_getItemsAsjQuery : function(M) {
									var O, N, T, Q, R = [], P = [], S = this
											._connectWith();
									if (S && M) {
											...
											for (N = T.length - 1; N >= 0; N--) {
												...
												if (Q && Q !== this
													...
													P
															.push([
																	y
																			.isFunction(Q.options.items) ? Q.options.items
																			.call(Q.element)
																			: y(
																					Q.options.items,
																					Q.element)
								...
								_removeCurrentsFromItems : function() {
									var M = this.currentItem.find(":data("
								...
								_refreshItems : function(M) {
									...
									var Q, O, V, R, U, N, X, W, S = this.items, P = [ [
											y.isFunction(this.options.items) ? this.options.items
													.call(this.element[0], M, {
														item : this.currentItem
													})
													: y(this.options.items,
															this.element), this ] ], T = this
											._connectWith();
									if (T && this.ready) {
											...
											for (O = V.length - 1; O >= 0; O--) {
												...
												if (R && R !== this
													...
													P
															.push([
																	y
																			.isFunction(R.options.items) ? R.options.items
																			.call(
																					R.element[0],
																					M,
																					{
								...
								refreshPositions : function(M) {
									...
									for (O = this.items.length - 1; O >= 0; O--) {
										...
										if (P.instance !== this.currentContainer
												...
												&& P.item[0] !== this.currentItem[0]) {
											continue
										}
										N = this.options.toleranceElement ? y(
									...
									if (this.options.custom
										...
										for (O = this.containers.length - 1; O >= 0; O--) {
											Q = this.containers[O].element
													.offset();
											this.containers[O].containerCache.left = Q.left;
											this.containers[O].containerCache.top = Q.top;
											this.containers[O].containerCache.width = this.containers[O].element
													.outerWidth();
											this.containers[O].containerCache.height = this.containers[O].element
													.outerHeight()
										}
									}
									return this
								...
								_createPlaceholder : function(N) {
									...
									if (!O.placeholder
										...
										O.placeholder = {
											element : function() {
												var Q = N.currentItem[0].nodeName
														.toLowerCase(), P = y(
														"<" + Q + ">",
														N.document[0])
														.addClass(
									...
									N.placeholder = y(O.placeholder.element
											.call(N.element, N.currentItem));
								...
								_contactContainers : function(M) {
									...
									for (R = this.containers.length - 1; R >= 0; R--) {
										if (y.contains(this.currentItem[0],
												this.containers[R].element[0])) {
											continue
										}
										if (this
												._intersectsWith(this.containers[R].containerCache)) {
											if (N
													&& y
															.contains(
																	this.containers[R].element[0],
									...
									if (this.containers.length === 1) {
										if (!this.containers[W].containerCache.over) {
											this.containers[W]._trigger("over",
													M, this._uiHash(this));
										...
										S ? this._rearrange(M, S, null, true)
												: this
														._rearrange(
																M,
																null,
																this.containers[W].element,
																true);
										this._trigger("change", M, this
												._uiHash());
										this.containers[W]._trigger("change",
												M, this._uiHash(this));
										this.currentContainer = this.containers[W];
										this.options.placeholder.update(
												this.currentContainer,
												this.placeholder);
										this.containers[W]._trigger("over", M,
												this._uiHash(this));
										this.containers[W].containerCache.over = 1
									}
								},
								_createHelper : function(N) {
									var O = this.options, M = y
											...
											: (O.helper === "clone" ? this.currentItem
													...
													: this.currentItem);
									if (!M.parents("body").length) {
										y(O.appendTo !== "parent" ? O.appendTo
								...
								_convertPositionTo : function(O, Q) {
									...
									var N = O === "absolute" ? 1 : -1, M = this.cssPosition === "absolute"
											&& !(this.scrollParent[0] !== document && y
													.contains(
															this.scrollParent[0],
															this.offsetParent[0])) ? this.offsetParent
											: this.scrollParent, P = (/(html|body)/i)
											.test(M[0].tagName);
									return {
										top : (Q.top + this.offset.relative.top
												* N + this.offset.parent.top
												* N - ((this.cssPosition === "fixed" ? -this.scrollParent
												.scrollTop()
												: (P ? 0 : M.scrollTop())) * N)),
										left : (Q.left
												+ this.offset.relative.left * N
												+ this.offset.parent.left * N - ((this.cssPosition === "fixed" ? -this.scrollParent
												.scrollLeft()
												: P ? 0 : M.scrollLeft()) * N))
									}
								},
								_generatePosition : function(P) {
									var R, Q, S = this.options, O = P.pageX, N = P.pageY, M = this.cssPosition === "absolute"
											&& !(this.scrollParent[0] !== document && y
													.contains(
															this.scrollParent[0],
															this.offsetParent[0])) ? this.offsetParent
											: this.scrollParent, T = (/(html|body)/i)
											.test(M[0].tagName);
									if (this.cssPosition === "relative"
									...
									if (this.originalPosition) {
										if (this.containment) {
											if (P.pageX
												...
												O = this.containment[0]
														+ this.offset.click.left
											}
											if (P.pageY - this.offset.click.top < this.containment[1]) {
												N = this.containment[1]
														+ this.offset.click.top
											}
											if (P.pageX
													- this.offset.click.left > this.containment[2]) {
												O = this.containment[2]
														+ this.offset.click.left
											}
											if (P.pageY - this.offset.click.top > this.containment[3]) {
												N = this.containment[3]
														+ this.offset.click.top
											}
										}
										if (S.grid) {
											R = this.originalPageY
													+ Math
															.round((N - this.originalPageY)
																	/ S.grid[1])
													* S.grid[1];
											N = this.containment ? ((R
													- this.offset.click.top >= this.containment[1] && R
													- this.offset.click.top <= this.containment[3]) ? R
													: ((R
															- this.offset.click.top >= this.containment[1]) ? R
															- S.grid[1]
															: R + S.grid[1]))
													: R;
											Q = this.originalPageX
													+ Math
															.round((O - this.originalPageX)
																	/ S.grid[0])
													* S.grid[0];
											O = this.containment ? ((Q
													- this.offset.click.left >= this.containment[0] && Q
													- this.offset.click.left <= this.containment[2]) ? Q
													: ((Q
															- this.offset.click.left >= this.containment[0]) ? Q
															- S.grid[0]
															: Q + S.grid[0]))
									...
									return {
										top : (N - this.offset.click.top
												- this.offset.relative.top
												- this.offset.parent.top + ((this.cssPosition === "fixed" ? -this.scrollParent
												.scrollTop()
												: (T ? 0 : M.scrollTop())))),
										left : (O - this.offset.click.left
												- this.offset.relative.left
												- this.offset.parent.left + ((this.cssPosition === "fixed" ? -this.scrollParent
												.scrollLeft()
												: T ? 0 : M.scrollLeft())))
									}
								},
								_rearrange : function(Q, P, N, O) {
									N ? N[0].appendChild(this.placeholder[0])
											: P.item[0].parentNode
													.insertBefore(
															this.placeholder[0],
															(this.direction === "down" ? P.item[0]
																	: P.item[0].nextSibling));
								...
								_clear : function(N, P) {
									...
									if (this.helper[0] === this.currentItem[0]) {
										for (M in this._storedCSS) {
											if (this._storedCSS[M] === "auto"
													|| this._storedCSS[M] === "static") {
												this._storedCSS[M] = ""
											}
										}
									...
									if (this.cancelHelperRemoval) {
										if (!P) {
											this._trigger("beforeStop", N, this
													._uiHash());
											for (M = 0; M < Q.length; M++) {
												Q[M].call(this, N)
											}
											this._trigger("stop", N, this
													._uiHash())
										}
								...
								_trigger : function() {
									if (y.Widget.prototype._trigger.apply(this,
											arguments) === false) {
										this.cancel()
									}
								},
					...
					.extend(
							K.prototype,
							{
								...
								_attachDatepicker : function(P, M) {
									...
									//***MODIFICACIÓN CalendarMonthRange
									//si solo se deben mostrar tres meses atrás modificar minDate
									//y si restringir fechas futuras en caso necesario
									if(N.settings.restrictMinDate){
										var d = new Date();
										
										var mes = d.getMonth()+1;
										mes = ((mes - 2) <= 0) ? (12 + (mes-2)) : (mes-2)
										mes = (mes<10 ? '0' : '') + mes;
										
										var anio = d.getFullYear();
										anio = ((d.getMonth() - 2) <= 0) ? (anio - 1) : anio;
										
										$.extend(N.settings,N.settings,{minDate: "01/"+mes+"/"+anio});
									} 
									if(N.settings.restrictMaxDate){
										var d = new Date();
										...
										$.extend(N.settings,N.settings,{maxDate: dia+"/"+mes+"/"+d.getFullYear()});
									} 
									//***MODIFICACIÓN CalendarMonthRange
									
								},
								_newInst : function(N, M) {
									var O = N[0].id.replace(
											/([^A-Za-z0-9_\-])/g, "\\\\$1");
									return {
										id : O,
										input : N,
								...
								_connectDatepicker : function(O, N) {
									...
									M.addClass(this.markerClassName).keydown(
								...
								_attachments : function(O, R) {
									var N, Q, M, S = this._get(R, "appendText"), P = this
											._get(R, "isRTL");
									if (R.append) {
										R.append.remove()
									}
									if (S) {
										R.append = y("<span class='"
												+ this._appendClass + "'>" + S
												+ "</span>");
									...
									if (N === "button" || N === "both") {
										...
										R.trigger = y(this._get(R,
												"buttonImageOnly") ? y("<img/>")
								...
								_autoSize : function(S) {
									if (this._get(S, "autoSize") && !S.inline) {
										var P, N, O, R, Q = new Date(2009,
												12 - 1, 20), M = this._get(S,
												"dateFormat");
										if (M.match(/[DM]/)) {
											P = function(T) {
												N = 0;
											...
											Q.setMonth(P(this._get(S, (M
													.match(/MM/) ? "monthNames"
													: "monthNamesShort"))));
											Q.setDate(P(this._get(S, (M
													.match(/DD/) ? "dayNames"
													: "dayNamesShort")))
													+ 20 - Q.getDay())
										}
										S.input.attr("size", this._formatDate(
												S, Q).length)
									}
								},
								_inlineDatepicker : function(N, M) {
									...
									O.addClass(this.markerClassName).append(
											M.dpDiv);
									y.data(N, "datepicker", M);
									this._setDate(M, this._getDefaultDate(M),
											true);
								...
								_dialogDatepicker : function(T, N, R, O, S) {
									...
									if (!P) {
										this.uuid += 1;
										M = "dp" + this.uuid;
										this._dialogInput = y("<input type='text' id='"
												+ M
										...
										P.settings = {};
										y.data(this._dialogInput[0],
												"datepicker", P)
									}
									p(P.settings, O || {});
									N = (N && N.constructor === Date ? this
											._formatDate(P, N) : N);
									this._dialogInput.val(N);
									this._pos = (S ? (S.length ? S : [ S.pageX,
											S.pageY ]) : null);
									if (!this._pos) {
									...
									y.data(this._dialogInput[0], "datepicker",
											P);
									return this
								},
								_destroyDatepicker : function(O) {
									...
									if (P === "input") {
										N.append.remove();
										N.trigger.remove();
										M.removeClass(this.markerClassName)
												.unbind("focus",
								...
								_enableDatepicker : function(P) {
									...
									this._disabledInputs = y.map(
											this._disabledInputs, function(R) {
												return (R === P ? null : R)
											})
								},
								...
								_optionDatepicker : function(S, N, R) {
									...
									if (arguments.length === 2
											&& typeof N === "string") {
										return (N === "defaults" ? y.extend({},
												y.datepicker._defaults)
												: (P ? (N === "all" ? y.extend(
														{}, P.settings) : this
														._get(P, N)) : null))
									}
									...
									if (P) {
										...
										if (Q !== null
												...
												&& O.minDate === undefined) {
											P.settings.minDate = this
													._formatDate(P, Q)
										}
										if (T !== null
												&& O.dateFormat !== undefined
												&& O.maxDate === undefined) {
											P.settings.maxDate = this
													._formatDate(P, T)
										}
										if ("disabled" in O) {
											if (O.disabled) {
								...
								_doKeyDown : function(P) {
									var N, M, R, Q = y.datepicker
											._getInst(P.target), S = true, O = Q.dpDiv
									...
									if (y.datepicker._datepickerShowing) {
										...
										case 13:
											R = y(
													"td."
															+ y.datepicker._dayOverClass
															+ ":not(."
															+ y.datepicker._currentClass
															+ ")", Q.dpDiv);
											if (R[0]) {
												y.datepicker._selectDay(
														P.target,
														Q.selectedMonth,
														Q.selectedYear, R[0])
											}
								...
								_doKeyPress : function(O) {
									var N, M, P = y.datepicker
											._getInst(O.target);
									if (y.datepicker._get(P, "constrainInput")) {
										N = y.datepicker
												._possibleChars(y.datepicker
														._get(P, "dateFormat"));
										M = String
												.fromCharCode(O.charCode == null ? O.keyCode
														: O.charCode);
										return O.ctrlKey
												|| O.metaKey
												|| (M < " " || !N || N
								...
								_showDatepicker : function(N) {
									...
									if (y.datepicker._isDisabledDatepicker(N)
									...
									if (y.datepicker._curInst
										...
										y.datepicker._curInst.dpDiv.stop(true,
												true);
										if (P
												&& y.datepicker._datepickerShowing) {
											y.datepicker
													._hideDatepicker(y.datepicker._curInst.input[0])
										}
									}
									...
									if (!y.datepicker._pos) {
										y.datepicker._pos = y.datepicker
												._findPos(N);
									...
									P.dpDiv.css({
										position : (y.datepicker._inDialog
												&& y.blockUI ? "static"
												: (R ? "fixed" : "absolute")),
										display : "none",
										left : S.left + "px",
								...
								_updateDatepicker : function(O) {
									this.maxRows = 4;
									j = O;
									O.dpDiv.empty().append(
											this._generateHTML(O));
									this._attachHandlers(O);
									O.dpDiv.find("." + this._dayOverClass
											+ " a");
									...
									if (P > 1) {
										O.dpDiv.addClass(
												"ui-datepicker-multi-" + P)
												.css("width", (N * P) + "em")
									}
									O.dpDiv[(M[0] !== 1 || M[1] !== 1 ? "add"
											: "remove")
											+ "Class"]("ui-datepicker-multi");
									O.dpDiv[(this._get(O, "isRTL") ? "add"
											: "remove")
											+ "Class"]("ui-datepicker-rtl");
									if (O === y.datepicker._curInst
											&& y.datepicker._datepickerShowing
											&& y.datepicker
													._shouldFocusInput(O)) {
										O.input.focus()
								...
								_shouldFocusInput : function(M) {
									return M.input && M.input.is(":visible")
											&& !M.input.is(":disabled")
											&& !M.input.is(":focus")
								},
								_checkOffset : function(R, P, O) {
									var Q = R.dpDiv.outerWidth(), U = R.dpDiv
											.outerHeight(), T = R.input ? R.input
											.outerWidth()
											: 0, M = R.input ? R.input
											.outerHeight() : 0, S = document.documentElement.clientWidth
											+ (O ? 0 : y(document).scrollLeft()), N = document.documentElement.clientHeight
											+ (O ? 0 : y(document).scrollTop());
									P.left -= (this._get(R, "isRTL") ? (Q - T)
											: 0);
									P.left -= (O && P.left === R.input.offset().left) ? y(
											document).scrollLeft()
											: 0;
									P.top -= (O && P.top === (R.input.offset().top + M)) ? y(
											document).scrollTop()
											: 0;
									P.left -= Math.min(P.left,
											(P.left + Q > S && S > Q) ? Math
													.abs(P.left + Q - S) : 0);
									P.top -= Math.min(P.top,
											(P.top + U > N && N > U) ? Math
													.abs(U + M) : 0);
									return P
								},
								_findPos : function(P) {
									var M, O = this._getInst(P), N = this._get(
											O, "isRTL");
									while (P
											&& (P.type === "hidden"
													|| P.nodeType !== 1 || y.expr.filters
													.hidden(P))) {
										P = P[N ? "previousSibling"
												: "nextSibling"]
									}
									M = y(P).offset();
									return [ M.left, M.top ]
								...
								_hideDatepicker : function(O) {
									...
									if (this._datepickerShowing) {
										...
										if (y.effects
												&& (y.effects.effect[N] || y.effects[N])) {
											P.dpDiv.hide(N, y.datepicker._get(
													P, "showOptions"), R, Q)
										} else {
											P.dpDiv[(N === "slideDown" ? "slideUp"
													: (N === "fadeIn" ? "fadeOut"
															: "hide"))]((N ? R
													: null), Q)
										}
								...
								_tidyDialog : function(M) {
									M.dpDiv.removeClass(this._dialogClass)
								...
								_checkExternalClick : function(N) {
									...
									var M = y(N.target), O = y.datepicker
											._getInst(M[0]);
									if (((M[0].id !== y.datepicker._mainDivId
											&& M.parents("#"
													+ y.datepicker._mainDivId).length === 0
											&& !M
													.hasClass(y.datepicker.markerClassName)
											&& !M
													.closest("."
															+ y.datepicker._triggerClass).length
											&& y.datepicker._datepickerShowing && !(y.datepicker._inDialog && y.blockUI)))
											|| (M
													.hasClass(y.datepicker.markerClassName) && y.datepicker._curInst !== O)) {
										y.datepicker._hideDatepicker()
									}
								},
								_adjustDate : function(Q, P, O) {
									var N = y(Q), M = this._getInst(N[0]);
								...
								_gotoToday : function(P) {
									...
									if (this._get(N, "gotoCurrent")
										...
										N.drawMonth = N.selectedMonth = M
												.getMonth();
										N.drawYear = N.selectedYear = M
												.getFullYear()
									}
								...
								_selectMonthYear : function(Q, M, P) {
									var O = y(Q), N = this._getInst(O[0]);
									N["selected"
											+ (P === "M" ? "Month" : "Year")] = N["draw"
											+ (P === "M" ? "Month" : "Year")] = parseInt(
								...
								_selectDay : function(R, P, M, Q) {
									var N, O = y(R);
									if (y(Q).hasClass(this._unselectableClass)
											|| this._isDisabledDatepicker(O[0])) {
										return
									}
									N = this._getInst(O[0]);
									N.selectedDay = N.currentDay = y("a", Q)
											.html();
									N.selectedMonth = N.currentMonth = P;
									N.selectedYear = N.currentYear = M;
									this._selectDate(R, this._formatDate(N,
											N.currentDay, N.currentMonth,
											N.currentYear))
								},
								...
								_updateAlternate : function(Q) {
									...
									if (N) {
										P = this._get(Q, "altFormat")
												|| this._get(Q, "dateFormat");
										O = this._getDate(Q);
										M = this.formatDate(P, O, this
												._getFormatConfig(Q));
										y(N).each(function() {
											y(this).val(M)
										})
								...
								iso8601Week : function(M) {
									...
									return Math.floor(Math
											.round((N - O) / 86400000) / 7) + 1
								},
								parseDate : function(ac, X, ae) {
									if (ac == null || X == null) {
										throw "Invalid arguments"
									}
									X = (typeof X === "object" ? X.toString()
											: X + "");
									if (X === "") {
										return null
									}
									var P, Z, N, ad = 0, S = (ae ? ae.shortYearCutoff
											: null)
											|| this._defaults.shortYearCutoff, O = (typeof S !== "string" ? S
											: new Date().getFullYear() % 100
													+ parseInt(S, 10)), V = (ae ? ae.dayNamesShort
											: null)
											|| this._defaults.dayNamesShort, ag = (ae ? ae.dayNames
											: null)
											|| this._defaults.dayNames, M = (ae ? ae.monthNamesShort
											: null)
											|| this._defaults.monthNamesShort, Q = (ae ? ae.monthNames
											: null)
											|| this._defaults.monthNames, R = -1, ah = -1, ab = -1, U = -1, aa = false, af, W = function(
											aj) {
										var ak = (P + 1 < ac.length && ac
												.charAt(P + 1) === aj);
										if (ak) {
											P++
										}
										...
										var aj = W(al), am = (al === "@" ? 14
												: (al === "!" ? 20
														: (al === "y" && aj ? 4
																: (al === "o" ? 3
																		: 2)))), an = new RegExp(
												"^\\d{1," + am + "}"), ak = X
												.substring(ad).match(an);
										if (!ak) {
									...
									af = this._daylightSavingAdjust(new Date(R,
											ah - 1, ab));
									if (af.getFullYear() !== R
											|| af.getMonth() + 1 !== ah
											|| af.getDate() !== ab) {
								...
								formatDate : function(V, P, Q) {
									...
									var X, Y = (Q ? Q.dayNamesShort : null)
											|| this._defaults.dayNamesShort, N = (Q ? Q.dayNames
											: null)
											|| this._defaults.dayNames, T = (Q ? Q.monthNamesShort
											: null)
											|| this._defaults.monthNamesShort, R = (Q ? Q.monthNames
											: null)
											|| this._defaults.monthNames, W = function(
											Z) {
										var aa = (X + 1 < V.length && V
												.charAt(X + 1) === Z);
										if (aa) {
											X++
										}
								...
								_get : function(N, M) {
									return N.settings[M] !== undefined ? N.settings[M]
											: this._defaults[M]
								},
								_setDateFromField : function(R, O) {
									...
									var M = this._get(R, "dateFormat"), T = R.lastVal = R.input ? R.input
											.val()
											: null, S = this._getDefaultDate(R), N = S, P = this
											._getFormatConfig(R);
									try {
									...
									R.drawMonth = R.selectedMonth = N
											.getMonth();
									R.drawYear = R.selectedYear = N
											.getFullYear();
								...
								_determineDate : function(Q, N, R) {
									var P = function(T) {
										...
										var T = (Z.toLowerCase().match(/^c/) ? y.datepicker
												._getDate(Q)
												: null)
												|| new Date(), U = T
												.getFullYear(), X = T
												.getMonth(), S = T.getDate(), W = /([+\-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, V = W
												.exec(Z);
										while (V) {
											switch (V[2] || "d") {
											case "d":
										...
										return new Date(U, X, S)
											...
											: (typeof N === "string" ? O(N)
													: (typeof N === "number" ? (isNaN(N) ? R
															: P(N))
															: new Date(N
																	.getTime()))));
									M = (M && M.toString() === "Invalid Date" ? R
											: M);
									if (M) {
										M.setHours(0);
										M.setMinutes(0);
								...
								_daylightSavingAdjust : function(M) {
									...
									M
											.setHours(M.getHours() > 12 ? M
													.getHours() + 2 : 0);
									return M
								},
								_setDate : function(S, P, R) {
									var M = !P, O = S.selectedMonth, Q = S.selectedYear, N = this
											._restrictMinMax(S, this
													._determineDate(S, P,
															new Date()));
									S.selectedDay = S.currentDay = N.getDate();
									S.drawMonth = S.selectedMonth = S.currentMonth = N
											.getMonth();
									S.drawYear = S.selectedYear = S.currentYear = N
											.getFullYear();
									if ((O !== S.selectedMonth || Q !== S.selectedYear)
											&& !R) {
								...
								_attachHandlers : function(N) {
									var M = this._get(N, "stepMonths"), O = "#"
											+ N.id.replace(/\\\\/g, "\\");
									N.dpDiv
											.find("[data-handler]")
											.map(
													function() {
														var P = {
															...
															//***MODIFICACIÓN CalendarMonthRange
															selectMonthRange : function() {
																
																var startId="";
																var endId ="";
																if(y.datepicker._curInst.id.toLowerCase().indexOf("start") >= 0){
																	var startId = y.datepicker._curInst.id;
																	var endId = startId.replace("Start","End").replace("\\","");
																}else if(y.datepicker._curInst.id.toLowerCase().indexOf("end") >= 0){
																	endId = y.datepicker._curInst.id.replace("\\","");
																	startId = endId.replace("End","Start");
																}
																
																
																
																//si NO se encuentra desahabilitado el campo de fin
//																if( $("#"+endId).attr('disabled') != "disabled" ){
																	var month = this.dataset.month-1;
																	var year =  this.dataset.year;
																	var lastDay = new Date(year,month +1, 0).getDate();
																	y.datepicker._curInst.selectedDay=1;
																	y.datepicker._curInst.selectedMonth=month;
																	y.datepicker._curInst.selectedYear=year;
																	y.datepicker._selectDate(O, y.datepicker._formatDate(N,
																			1, month,
																			year));

																	
																	$("#"+endId).val(y.datepicker._formatDate(N,
																			lastDay, month,
																			year))
...
//																	$("#"+endId).val("");
//																	$("#"+startId).val("");
//																	//linea agregada para esconder al deseleccionar mes
//																	//ya que el evento no se disparaba en automático
...
//																
								...
								_generateHTML : function(aD) {
									var af, ae, ay, ap, Q, aH, aB, au, aK, an, aO, X, Z, Y, N, aG, V, ai, aJ, aw, aP, ah, am, W, R, az, ar, av, at, U, ak, aa, aC, aF, P, aI, aM, aq, ab, aE = new Date(), ag = this
											._daylightSavingAdjust(new Date(aE
													.getFullYear(), aE
													.getMonth(), aE.getDate())), aL = this
											._get(aD, "isRTL"), aN = this._get(
											aD, "showButtonPanel"), ax = this
											._get(aD, "hideIfNoPrevNext"), al = this
											._get(aD, "navigationAsDateFormat"), ac = this
											._getNumberOfMonths(aD), T = this
											._get(aD, "showCurrentAtPos"), ao = this
											._get(aD, "stepMonths"), aj = (ac[0] !== 1 || ac[1] !== 1), O = this
											._daylightSavingAdjust((!aD.currentDay ? new Date(
													9999, 9, 9)
													: new Date(aD.currentYear,
															aD.currentMonth,
															aD.currentDay))), S = this
											._getMinMaxDate(aD, "min"), ad = this
											._getMinMaxDate(aD, "max"), M = aD.drawMonth
											- T, aA = aD.drawYear;
									//***MODIFICACIÓN CalendarMonthRange
									var startId="";
									var endId ="";
									if(aD.id.toLowerCase().indexOf("start") >= 0){										
										startId = aD.id.replace("\\","");
										endId = startId.replace("Start","End");
									}else if(aD.id.toLowerCase().indexOf("end") >= 0){
										endId =  aD.id.replace("\\","");
										startId = endId.replace("End","Start");
									}
									//***MODIFICACIÓN CalendarMonthRange
									...
									ae = (!al ? ae : this.formatDate(ae, this
											._daylightSavingAdjust(new Date(aA,
													M - ao, 1)), this
											._getFormatConfig(aD)));
									ay = (this._canAdjustMonth(aD, -1, aA, M) ? "<a class='ui-datepicker-prev ui-corner-all' data-handler='prev' data-event='click' title='"
									...
									ap = (!al ? ap : this.formatDate(ap, this
											._daylightSavingAdjust(new Date(aA,
													M + ao, 1)), this
											._getFormatConfig(aD)));
									Q = (this._canAdjustMonth(aD, +1, aA, M) ? "<a class='ui-datepicker-next ui-corner-all' data-handler='next' data-event='click' title='"
											+ ap
											+ "'><span class='ui-icon ui-icon-circle-triangle-"
											+ (aL ? "w" : "e")
											+ "'>"
									...
									aB = (this._get(aD, "gotoCurrent")
											&& aD.currentDay ? O : ag);
									aH = (!al ? aH : this.formatDate(aH, aB,
											this._getFormatConfig(aD)));
									//***MODIFICACIÓN CalendarMonthRange
//									au = (!aD.inline ? "<button type='button' class='ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all' data-handler='hide' data-event='click'>"
//											+ this._get(aD, "closeText")
//											+ "</button>"
//											: "");
									var textoBoton2 = "";
									var textoBoton1 = "";
									...
									if(mesActual > 3){
										monthBoton2 = mesActual+1;
										monthBoton1 = mesActual;
										monthBoton0 = mesActual-1;
										yearBoton2 = year;
										yearBoton1 = year;
										yearBoton0 = year;
									}else{
										monthBoton2 = (mesActual+1);
										monthBoton1 = ((mesActual) > 0) ? mesActual : mesActual+12;
										monthBoton0 = (mesActual-1) > 0? mesActual-1 : mesActual+11;
										yearBoton2 = year;
										yearBoton1 = ((mesActual) > 0) ? year : year-1;
										yearBoton0 = (mesActual-1) > 0? year : year-1;
									}
									meses = this._get(aD, "monthNames");
									textoBoton2 = meses[monthBoton2-1];
									textoBoton1 = meses[monthBoton1-1];
									textoBoton0 = meses[monthBoton0-1];
									
									aK = (aN) ? "<div class='ui-datepicker-buttonpane ui-widget-content'>"
//											+ (aL ? au : "")
//											+ (this._isInRange(aD, aB) ? "<button type='button' class='ui-datepicker-current ui-state-default ui-priority-secondary ui-corner-all' data-handler='today' data-event='click'>"
//													+ aH + "</button>"
//													: "")
											
											+"<button type='button' id='ui-month-button-2' data-month='"+monthBoton2+"' data-year='"+yearBoton2+"' class='ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all' data-handler='selectMonthRange' data-event='click'>"
											+ textoBoton2+ " " +(yearBoton2==year?"":yearBoton2)
											+ "</button>" 
											+"<button type='button' id='ui-month-button-1' data-month='"+monthBoton1+"' data-year='"+yearBoton1+"' class='ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all' data-handler='selectMonthRange' data-event='click'>"
											+ textoBoton1+ " " +(yearBoton1==year?"":yearBoton1)
											+ "</button>" 
											+"<button type='button' id='ui-month-button-0' data-month='"+monthBoton0+"' data-year='"+yearBoton0+"' class='ui-datepicker-close ui-state-default ui-priority-primary ui-corner-all' data-handler='selectMonthRange' data-event='click'>"
											+ textoBoton0+ " " +(yearBoton0==year?"":yearBoton0)
											+ "</button>" 
//											+ (aL ? "" : au)
											+ "</div>"
											: "";
									...
									for (ah = 0; ah < ac[0]; ah++) {
										...
										for (W = 0; W < ac[1]; W++) {
											...
											ar += "<div class='ui-datepicker-header ui-widget-header ui-helper-clearfix"
													+ az
													+ "'>"
													+ (/all|left/.test(az)
															&& ah === 0 ? (aL ? Q
											...
											av = (aO ? "<th class='ui-datepicker-week-col'>"
													+ this._get(aD,
															"weekHeader")
													+ "</th>"
													: "");
											for (aP = 0; aP < 7; aP++) {
												at = (aP + an) % 7;
												av += "<th scope='col'"
														+ ((aP + an + 6) % 7 >= 5 ? " class='ui-datepicker-week-end'"
																: "")
														+ "><span title='"
														+ X[at] + "'>" + Z[at]
														+ "</span></th>"
											...
											if (aA === aD.selectedYear
													&& M === aD.selectedMonth) {
												aD.selectedDay = Math.min(
														aD.selectedDay, U)
											}
											ak = (this._getFirstDayOfMonth(aA,
													M)
													- an + 7) % 7;
											aa = Math.ceil((ak + U) / 7);
											aC = (aj ? this.maxRows > aa ? this.maxRows
													: aa
													: aa);
											this.maxRows = aC;
											aF = this
											...
											for (P = 0; P < aC; P++) {
												...
												for (aP = 0; aP < 7; aP++) {
													...
													//***MODIFICACIÓN CalendarMonthRange
													//poner estilo de seleccionado a los días del rango
													var estiloTD = "";
													if( ($("#"+startId).length>0 && $("#"+startId).val() !="") && ($("#"+endId).length>0 && $("#"+endId).val() != "")){
														
														var fechaInicio = $("#"+startId).val();
														var fechaFin = $("#"+endId).val();
														var inicioSplit = fechaInicio.split("/");
														var finSplit = fechaFin.split("/");
														var dInicio = new Date(inicioSplit[2]+"/"+inicioSplit[1]+"/"+inicioSplit[0]);
														var dFin = new Date(finSplit[2]+"/"+finSplit[1]+"/"+finSplit[0]);
														if( (dInicio.getTime() <= aF.getTime()) && (aF.getTime() <= dFin.getTime()) ){
															estiloTD = " ui-rangoSeleccionado";
														}
														if(dInicio.getTime() == aF.getTime()){
															estiloTD = " ui-inicioRango";
														}
														if(dFin.getTime() == aF.getTime()){
															estiloTD = " ui-finRango";
														}
													}
													//***MODIFICACIÓN CalendarMonthRange
													aI += "<td class='"
															+ ((aP + an + 6) % 7 >= 5 ? " ui-datepicker-week-end"
																	: "")
															+ (aq ? " ui-datepicker-other-month"
																	: "")
															+ ((aF.getTime() === R
																	.getTime()
																	&& M === aD.selectedMonth && aD._keyEvent)
																	|| (aJ
																			.getTime() === aF
																			.getTime() && aJ
																			.getTime() === R
															...
															+ ">"
															+ (aq && !V ? "&#xa0;"
																	: (ab ? "<span class='ui-state-default'>"
																			+ aF
																					.getDate()
																			+ "</span>"
											...
											ar += "</tbody></table>"
													+ (aj ? "</div>"
															+ ((ac[0] > 0 && W === ac[1] - 1) ? "<div class='ui-datepicker-row-break'></div>"
																	: "")
															: "");
											am += ar
										}
								...
								_generateMonthYearHeader : function(Q, O, Y, S,
										W, Z, U, M) {
									var ad, N, ae, ab, R, aa, X, T, P = this
											._get(Q, "changeMonth"), af = this
											._get(Q, "changeYear"), ag = this
											._get(Q, "showMonthAfterYear"), V = "<div class='ui-datepicker-title'>", ac = "";
									if (Z || !P) {
										ac += "<span class='ui-datepicker-month'>"
												+ U[O] + "</span>"
									} else {
									...
									if (!Q.yearshtml) {
										Q.yearshtml = "";
										if (Z || !af) {
											V += "<span class='ui-datepicker-year'>"
													+ Y + "</span>"
										} else {
											ab = this._get(Q, "yearRange")
													.split(":");
											R = new Date().getFullYear();
											aa = function(ai) {
												var ah = (ai.match(/c[+\-].*/) ? Y
								...
								_adjustInstDate : function(P, S, R) {
									var O = P.drawYear + (R === "Y" ? S : 0), Q = P.drawMonth
											+ (R === "M" ? S : 0), M = Math
											.min(P.selectedDay, this
													._getDaysInMonth(O, Q))
											+ (R === "D" ? S : 0), N = this
											._restrictMinMax(
													P,
									...
									P.drawMonth = P.selectedMonth = N
											.getMonth();
									P.drawYear = P.selectedYear = N
											.getFullYear();
									if (R === "M" || R === "Y") {
								...
								_restrictMinMax : function(P, N) {
									var O = this._getMinMaxDate(P, "min"), Q = this
											._getMinMaxDate(P, "max"), M = (O
											&& N < O ? O : N);
									return (Q && M > Q ? Q : M)
								},
								...
								_getMinMaxDate : function(N, M) {
									return this._determineDate(N, this._get(N,
											M + "Date"), null)
								},
								_getDaysInMonth : function(M, N) {
									return 32 - this._daylightSavingAdjust(
											new Date(M, N, 32)).getDate()
								},
								_getFirstDayOfMonth : function(M, N) {
									return new Date(M, N, 1).getDay()
								},
								_canAdjustMonth : function(P, R, O, Q) {
									var M = this._getNumberOfMonths(P), N = this
											._daylightSavingAdjust(new Date(O,
													Q
															+ (R < 0 ? R : M[0]
																	* M[1]), 1));
									if (R < 0) {
										N.setDate(this._getDaysInMonth(N
												.getFullYear(), N.getMonth()))
									}
									return this._isInRange(P, N)
								},
								_isInRange : function(Q, O) {
									var N, T, P = this._getMinMaxDate(Q, "min"), M = this
											._getMinMaxDate(Q, "max"), U = null, R = null, S = this
											._get(Q, "yearRange");
									if (S) {
										N = S.split(":");
									...
									return ((!P || O.getTime() >= P.getTime())
											&& (!M || O.getTime() <= M
													.getTime())
											&& (!U || O.getFullYear() >= U) && (!R || O
											.getFullYear() <= R))
								},
								_getFormatConfig : function(M) {
									var N = this._get(M, "shortYearCutoff");
									N = (typeof N !== "string" ? N : new Date()
											.getFullYear()
											% 100 + parseInt(N, 10));
									return {
										shortYearCutoff : N,
										dayNamesShort : this._get(M,
												"dayNamesShort"),
										dayNames : this._get(M, "dayNames"),
										monthNamesShort : this._get(M,
												"monthNamesShort"),
									...
									var N = (M ? (typeof M === "object" ? M
											...
											: this
													._daylightSavingAdjust(new Date(
															...
															P.currentMonth,
															P.currentDay)));
									return this.formatDate(this._get(P,
											"dateFormat"), N, this
											._getFormatConfig(P))
								}
							});
			function g(N) {
				...
				return N.delegate(M, "mouseout", function() {
					...
					if (this.className.indexOf("ui-datepicker-next") !== -1) {
						...
						function() {
							if (!y.datepicker
									._isDisabledDatepicker(j.inline ? N
											.parent()[0] : j.input[0])) {
								y(this).parents(".ui-datepicker-calendar")
										.find("a")
			...
			y.fn.datepicker = function(N) {
				...
				if (typeof N === "string"
						&& (N === "isDisabled" || N === "getDate" || N === "widget")) {
					return y.datepicker["_" + N + "Datepicker"].apply(
							y.datepicker, [ this[0] ].concat(M))
				}
				if (N === "option" && arguments.length === 2
				...
				return this
						.each(function() {
							typeof N === "string" ? y.datepicker["_" + N
									+ "Datepicker"].apply(y.datepicker,
									[ this ].concat(M)) : y.datepicker
									._attachDatepicker(this, N)
						})
			};
			y.datepicker = new K();
			...
			(function(aa, P) {
				var W = "backgroundColor borderBottomColor borderLeftColor borderRightColor borderTopColor color columnRuleColor outlineColor textDecorationColor textEmphasisColor", T = /^([\-+])=\s*(\d+\.?\d*)/, S = [
						...
						},
						{
							...
							parse : function(ab) {
								return [ parseInt(ab[1] + ab[1], 16),
										parseInt(ab[2] + ab[2], 16),
										parseInt(ab[3] + ab[3], 16) ]
							}
						},
						{
				...
				function R(ab) {
					var ad = Q(), ac = ad._rgba = [];
					ab = ab.toLowerCase();
					X(S,
							function(ai, aj) {
								var ag, ah = aj.re.exec(ab), af = ah
										&& aj.parse(ah), ae = aj.space
										|| "rgba";
								if (af) {
									ag = ad[ae](af);
						...
						.extend(
								Q.prototype,
								{
									parse : function(ah, af, ab, ag) {
										...
										var ae = this, ad = aa.type(ah), ac = this._rgba = [];
										if (af !== P) {
											ah = [ ah, af, ab, ag ];
											ad = "array"
										}
										if (ad === "string") {
											return this.parse(R(ah)
													|| M._default)
										}
										if (ad === "array") {
									...
									transition : function(ac, ai) {
										var ad = Q(ac), ae = ad._space(), af = V[ae], ag = this
												.alpha() === 0 ? Q("transparent")
												: this, ah = ag[af.cache]
												|| af.to(ag._rgba), ab = ah
												.slice();
										ad = ad[af.cache];
										X(
												af.props,
												function(am, ao) {
													var al = ao.idx, ak = ah[al], aj = ad[al], an = Z[ao.type]
															|| {};
													if (aj === null) {
														return
													}
				...
				V.hsla.to = function(ad) {
					...
					var ab = ad[0] / 255, ag = ad[1] / 255, ah = ad[2] / 255, aj = ad[3], ai = Math
							.max(ab, ag, ah), ae = Math.min(ab, ag, ah), ak = ai
							- ae, al = ai + ae, ac = al * 0.5, af, am;
					if (ae === ai) {
						af = 0
					} else {
						if (ab === ai) {
				...
				V.hsla.from = function(af) {
					...
					var ae = af[0] / 360, ad = af[1], ac = af[2], ab = af[3], ag = ac <= 0.5 ? ac
							* (1 + ad)
							: ac + ad - ac * ad, ah = 2 * ac - ag;
					return [ Math.round(O(ah, ag, ae + (1 / 3)) * 255),
							Math.round(O(ah, ag, ae) * 255),
							Math.round(O(ah, ag, ae - (1 / 3)) * 255), ab ]
				};
				X(
						V,
						function(ac, ae) {
							var ad = ae.props, ab = ae.cache, ag = ae.to, af = ae.from;
							Q.fn[ac] = function(al) {
								if (ag && !this[ab]) {
								...
								var ai, ak = aa.type(al), ah = (ak === "array" || ak === "object") ? al
										: arguments, aj = this[ab].slice();
								X(ad, function(am, ao) {
							...
							X(
									...
									function(ah, ai) {
										...
										Q.fn[ah] = function(am) {
											var ao = aa.type(am), al = (ah === "alpha" ? (this._hsla ? "hsla"
													: "rgba")
													: ac), ak = this[al](), an = ak[ai.idx], aj;
											if (ao === "undefined") {
												return an
											}
											if (ao === "function") {
							...
							function(ad, ae) {
								aa.cssHooks[ae] = {
									set : function(ai, aj) {
										var ag, ah, af = "";
										if (aj !== "transparent"
												&& (aa.type(aj) !== "string" || (ag = R(aj)))) {
											aj = Q(ag || aj);
											if (!Y.rgba && aj._rgba[3] !== 1) {
												ah = ae === "backgroundColor" ? ai.parentNode
														: ai;
												while ((af === "" || af === "transparent")
														&& ah && ah.style) {
													try {
														af = aa
																.css(ah,
								...
								aa.fx.step[ae] = function(af) {
									...
									aa.cssHooks[ae].set(af.elem, af.start
											.transition(af.end, af.pos))
								}
							})
				};
				Q.hook(W);
			...
			(function() {
				...
				y.each([ "borderLeftStyle", "borderRightStyle",
						"borderBottomStyle", "borderTopStyle" ],
						function(Q, R) {
							y.fx.step[R] = function(S) {
								if (S.end !== "none" && !S.setAttr
										|| S.pos === 1 && !S.setAttr) {
				...
				function P(U) {
					var R, Q, S = U.ownerDocument.defaultView ? U.ownerDocument.defaultView
				...
				if (!y.fn.addBack) {
					y.fn.addBack = function(Q) {
						return this.add(Q == null ? this.prevObject
								: this.prevObject.filter(Q))
					}
				}
				y.effects.animateClass = function(Q, R, U, T) {
					...
					return this
							.queue(function() {
								var X = y(this), V = X.attr("class") || "", W, Y = S.children ? X
										.find("*").addBack()
										: X;
								Y = Y.map(function() {
									var Z = y(this);
						...
						.extend({
							addClass : (function(Q) {
								return function(S, R, U, T) {
									return R ? y.effects.animateClass.call(
											this, {
												add : S
											}, R, U, T) : Q.apply(this,
											arguments)
							...
							removeClass : (function(Q) {
								return function(S, R, U, T) {
									return arguments.length > 1 ? y.effects.animateClass
											.call(this, {
												remove : S
											}, R, U, T)
			...
			(function() {
				y.extend(y.effects, {
					...
					createWrapper : function(P) {
						...
						var Q = {
							...
							"float" : P.css("float")
						}, T = y("<div></div>").addClass("ui-effects-wrapper")
								.css({
									fontSize : "100%",
				...
				function M(P, O, Q, R) {
					...
					Q = Q || O.duration;
					P.duration = y.fx.off ? 0 : typeof Q === "number" ? Q
							: Q in y.fx.speeds ? y.fx.speeds[Q]
				...
				y.fn
						.extend({
							effect : function() {
								var Q = M.apply(this, arguments), S = Q.mode, O = Q.queue, P = y.effects.effect[Q.effect];
								if (y.fx.off || !P) {
									if (S) {
								...
								function R(V) {
									var W = y(this), U = Q.complete, X = Q.mode;
									function T() {
										if (y.isFunction(U)) {
											U.call(W[0])
										}
								...
								return O === false ? this.each(R) : this.queue(
										O || "fx", R)
							},
							show : (function(O) {
								return function(Q) {
			...
			(function() {
				var M = {};
				y.each([ "Quad", "Cubic", "Quart", "Quint", "Expo" ], function(
						O, N) {
					M[N] = function(P) {
				...
				y.extend(M, {
					...
					Bounce : function(P) {
						...
						return 1 / Math.pow(4, 3 - O) - 7.5625
								* Math.pow((N * 3 - 2) / 22 - P, 2)
					}
				});
			...
			var J = y.effects.effect.blind = function(O, U) {
				var P = y(this), Y = /up|down|vertical/, X = /up|left|vertical|horizontal/, Z = [
						"position", "top", "bottom", "left", "right", "height",
						"width" ], V = y.effects.setMode(P, O.mode || "hide"), aa = O.direction
						|| "up", R = Y.test(aa), Q = R ? "height" : "width", W = R ? "top"
						: "left", ac = X.test(aa), T = {}, ab = V === "show", N, M, S;
				if (P.parent().is(".ui-effects-wrapper")) {
				...
				T[Q] = ab ? M : 0;
				if (!ac) {
					P.css(R ? "bottom" : "right", 0).css(R ? "top" : "left",
							"auto").css({
						position : "absolute"
					});
					T[W] = ab ? S : M + S
			...
			var F = y.effects.effect.bounce = function(V, U) {
				var M = y(this), N = [ "position", "top", "bottom", "left",
						"right", "height", "width" ], T = y.effects.setMode(M,
						V.mode || "effect"), S = T === "hide", ad = T === "show", ae = V.direction
						|| "up", O = V.distance, R = V.times || 5, af = R * 2
						+ (ad || S ? 1 : 0), ac = V.duration / af, X = V.easing, P = (ae === "up" || ae === "down") ? "top"
						: "left", W = (ae === "up" || ae === "left"), ab, Q, aa, Y = M
						.queue(), Z = Y.length;
				if (ad || S) {
					N.push("opacity")
				}
				...
				if (ad) {
					...
					aa[P] = 0;
					M.css("opacity", 0).css(P, W ? -O * 2 : O * 2).animate(aa,
							ac, X)
				}
				if (S) {
					O = O / Math.pow(2, R - 1)
			...
			var t = y.effects.effect.clip = function(P, S) {
				var Q = y(this), W = [ "position", "top", "bottom", "left",
						"right", "height", "width" ], V = y.effects.setMode(Q,
						P.mode || "hide"), Y = V === "show", X = P.direction
						|| "vertical", U = X === "vertical", Z = U ? "height"
						: "width", T = U ? "top" : "left", R = {}, N, O, M;
				y.effects.save(Q, W);
				Q.show();
			...
			var l = y.effects.effect.drop = function(N, R) {
				var O = y(this), T = [ "position", "top", "bottom", "left",
						"right", "opacity", "height", "width" ], S = y.effects
						.setMode(O, N.mode || "hide"), V = S === "show", U = N.direction
						|| "left", P = (U === "up" || U === "down") ? "top"
						: "left", W = (U === "up" || U === "left") ? "pos"
						: "neg", Q = {
					opacity : V ? 1 : 0
				}, M;
				y.effects.save(O, T);
				...
				M = N.distance
						|| O[P === "top" ? "outerHeight" : "outerWidth"](true)
						/ 2;
				if (V) {
			...
			var E = y.effects.effect.explode = function(Z, Y) {
				var S = Z.pieces ? Math.round(Math.sqrt(Z.pieces)) : 3, N = S, M = y(this), U = y.effects
						.setMode(M, Z.mode || "hide"), ad = U === "show", Q = M
						.show().css("visibility", "hidden").offset(), aa = Math
						.ceil(M.outerWidth() / N), X = Math.ceil(M
						.outerHeight()
						/ S), R = [], ac, ab, O, W, V, T;
				function ae() {
					R.push(this);
			...
			var G = y.effects.effect.fade = function(P, M) {
				var N = y(this), O = y.effects.setMode(N, P.mode || "toggle");
				N.animate({
					opacity : O
				}, {
					queue : false,
			...
			var v = y.effects.effect.fold = function(O, S) {
				var P = y(this), X = [ "position", "top", "bottom", "left",
						"right", "height", "width" ], U = y.effects.setMode(P,
						O.mode || "hide"), aa = U === "show", V = U === "hide", ac = O.size || 15, W = /([0-9]+)%/
						.exec(ac), ab = !!O.horizFirst, T = aa !== ab, Q = T ? [
						"width", "height" ]
						: [ "height", "width" ], R = O.duration / 2, N, M, Z = {}, Y = {};
				y.effects.save(P, X);
				P.show();
				N = y.effects.createWrapper(P).css({
					overflow : "hidden"
			...
			var A = y.effects.effect.highlight = function(R, M) {
				var O = y(this), N = [ "backgroundImage", "backgroundColor",
						"opacity" ], Q = y.effects.setMode(O, R.mode || "show"), P = {
			...
			var a = y.effects.effect.size = function(V, U) {
				var Z, S, T, M = y(this), Y = [ "position", "top", "bottom",
						"left", "right", "width", "height", "overflow",
						"opacity" ], X = [ "position", "top", "bottom", "left",
						"right", "overflow", "opacity" ], W = [ "width",
						"height", "overflow" ], Q = [ "fontSize" ], ab = [
						"borderTopWidth", "borderBottomWidth", "paddingTop",
						"paddingBottom" ], N = [ "borderLeftWidth",
						"borderRightWidth", "paddingLeft", "paddingRight" ], R = y.effects
						.setMode(M, V.mode || "effect"), aa = V.restore
						|| R !== "effect", ae = V.scale || "both", ac = V.origin
						|| [ "middle", "center" ], ad = M.css("position"), O = aa ? Y
						: X, P = {
					height : 0,
					width : 0,
					outerHeight : 0,
				...
				if (ae === "box" || ae === "both") {
					if (T.from.y !== T.to.y) {
						O = O.concat(ab);
						M.from = y.effects.setTransition(M, ab, T.from.y,
								M.from);
					...
					if (T.from.x !== T.to.x) {
						O = O.concat(N);
						M.from = y.effects
								.setTransition(M, N, T.from.x, M.from);
				...
				if (ae === "content" || ae === "both") {
					if (T.from.y !== T.to.y) {
						...
						M.from = y.effects
								.setTransition(M, Q, T.from.y, M.from);
				...
				if (ae === "content" || ae === "both") {
					...
					M.find("*[width]").each(
							function() {
								...
								if (T.from.y !== T.to.y) {
									ag.from = y.effects.setTransition(ag, ab,
											T.from.y, ag.from);
									ag.to = y.effects.setTransition(ag, ab,
											T.to.y, ag.to)
								}
								if (T.from.x !== T.to.x) {
									ag.from = y.effects.setTransition(ag, N,
											T.from.x, ag.from);
									ag.to = y.effects.setTransition(ag, N,
											T.to.x, ag.to)
								}
								ag.css(ag.from);
								ag.animate(ag.to, V.duration, V.easing,
										function() {
											if (aa) {
			...
			var e = y.effects.effect.scale = function(M, P) {
				var N = y(this), V = y.extend(true, {}, M), Q = y.effects
						.setMode(N, M.mode || "effect"), R = parseInt(
						M.percent, 10)
						|| (parseInt(M.percent, 10) === 0 ? 0
								: (Q === "hide" ? 0 : 100)), T = M.direction
						|| "both", U = M.origin, O = {
					height : N.height(),
					width : N.width(),
			...
			var L = y.effects.effect.puff = function(T, M) {
				var R = y(this), S = y.effects.setMode(R, T.mode || "hide"), P = S === "hide", Q = parseInt(
						T.percent, 10) || 150, O = Q / 100, N = {
					height : R.height(),
					width : R.width(),
			...
			var x = y.effects.effect.pulsate = function(M, Q) {
				var O = y(this), T = y.effects.setMode(O, M.mode || "show"), X = T === "show", U = T === "hide", Y = (X || T === "hide"), V = ((M.times || 5) * 2)
						+ (Y ? 1 : 0), P = M.duration / V, W = 0, S = O.queue(), N = S.length, R;
				if (X || !O.is(":visible")) {
			...
			var r = y.effects.effect.shake = function(U, T) {
				var M = y(this), N = [ "position", "top", "bottom", "left",
						"right", "height", "width" ], S = y.effects.setMode(M,
						U.mode || "effect"), ac = U.direction || "left", O = U.distance || 20, R = U.times || 3, ad = R * 2 + 1, Y = Math
						.round(U.duration / ad), Q = (ac === "up" || ac === "down") ? "top"
						: "left", P = (ac === "up" || ac === "left"), ab = {}, aa = {}, Z = {}, X, V = M
						.queue(), W = V.length;
				y.effects.save(M, N);
				M.show();
				y.effects.createWrapper(M);
				...
				M.animate(aa, Y, U.easing).animate(ab, Y / 2, U.easing).queue(
						function() {
							if (S === "hide") {
								M.hide()
							}
			...
			var q = y.effects.effect.slide = function(O, S) {
				var P = y(this), U = [ "position", "top", "bottom", "left",
						"right", "width", "height" ], T = y.effects.setMode(P,
						O.mode || "show"), W = T === "show", V = O.direction
						|| "left", Q = (V === "up" || V === "down") ? "top"
						: "left", N = (V === "up" || V === "left"), M, R = {};
				y.effects.save(P, U);
				P.show();
				M = O.distance
						|| P[Q === "top" ? "outerHeight" : "outerWidth"](true);
			...
			var k = y.effects.effect.transfer = function(N, R) {
				var P = y(this), U = y(N.to), X = U.css("position") === "fixed", T = y("body"), V = X ? T
						.scrollTop()
						: 0, W = X ? T.scrollLeft() : 0, M = U.offset(), Q = {
					top : M.top - V,
					left : M.left - W,
					...
					width : U.innerWidth()
						...
						document.body).addClass(N.className).css({
					top : S.top - V,
					left : S.left - W,
...
(function($) {
					...
					Timepicker.prototype,
					{
							...
							if ($dp.find("div.ui-timepicker-div").length === 0
								...
								if (this._defaults.addSliderAccess) {
									...
									setTimeout(
											function() {
												if ($tp
														.find(".ui-selectoneradioslider-access").length === 0) {
													$tp
															.find(
																	".ui-selectoneradioslider:visible")
															.sliderAccess(
																	sliderAccessArgs);
													var sliderAccessWidth = $tp
															.find(
																	".ui-selectoneradioslider-access:eq(0)")
															.outerWidth(true);
						...
						_limitMinMaxDateTime : function(dp_inst, adjustSliders) {
							...
							if (dp_inst.settings.minTime !== null) {
								...
								if (this.hour < tempMinTime.getHours()) {
									this.hour = this._defaults.hourMin = tempMinTime
											.getHours();
									this.minute = this._defaults.minuteMin = tempMinTime
											.getMinutes()
								} else {
									if (this.hour === tempMinTime.getHours()
											&& this.minute < tempMinTime
													.getMinutes()) {
										this.minute = this._defaults.minuteMin = tempMinTime
												.getMinutes()
									} else {
										if (this._defaults.hourMin < tempMinTime
												.getHours()) {
											this._defaults.hourMin = tempMinTime
													.getHours();
							...
							if (dp_inst.settings.maxTime !== null) {
								...
								if (this.hour > tempMaxTime.getHours()) {
									this.hour = this._defaults.hourMax = tempMaxTime
											.getHours();
									this.minute = this._defaults.minuteMax = tempMaxTime
											.getMinutes()
								} else {
									if (this.hour === tempMaxTime.getHours()
											&& this.minute > tempMaxTime
													.getMinutes()) {
										this.minute = this._defaults.minuteMax = tempMaxTime
												.getMinutes()
									} else {
										if (this._defaults.hourMax > tempMaxTime
												.getHours()) {
											this._defaults.hourMax = tempMaxTime
													.getHours();
						...
						_updateDateTime : function(dp_inst) {
							...
							var dtTmp = (dp_inst.currentYear > 0 ? new Date(
									...
									._daylightSavingAdjust(dtTmp), dateFmt = $.datepicker
							...
							if (!this._defaults.showTimepicker) {
								...
								if (this.$altInput
									...
									if (this.$altInput) {
										...
										var altFormattedDateTime = "", altSeparator = this._defaults.altSeparator !== null ? this._defaults.altSeparator
												: this._defaults.separator, altTimeSuffix = this._defaults.altTimeSuffix !== null ? this._defaults.altTimeSuffix
												: this._defaults.timeSuffix;
					...
					if (typeof (o) === "string") {
						...
						return this
								.each(function() {
									var $t = $(this);
									$t
											.datepicker($.timepicker._newInst(
													$t, o)._defaults)
								})
					}
				}
	...
	var detectSupport = function(timeFormat) {
		var tf = timeFormat.replace(/'.*?'/g, "").toLowerCase(), isIn = function(
				f, t) {
	...
	var splitDateTime = function(dateTimeString, timeSettings) {
		var separator = computeEffectiveSetting(timeSettings, "separator"), format = computeEffectiveSetting(
				...
				.split(separator), timePartsLen = timeParts.length, allParts = dateTimeString
	...
	$.timepicker.timezoneOffsetNumber = function(tzString) {
		...
		return ((normalized.substr(0, 1) === "-" ? -1 : 1) * ((parseInt(
				normalized.substr(1, 2), 10) * 60) + parseInt(normalized
				.substr(3, 2), 10)))
	};
...
jQuery.cookie = function(d, e, b) {
	if (arguments.length > 1 && (e === null || typeof e !== "object")) {
		...
		return (document.cookie = [ encodeURIComponent(d), "=",
				b.raw ? String(e) : encodeURIComponent(String(e)),
	...
	return (a = new RegExp("(?:^|; )" + encodeURIComponent(d) + "=([^;]*)")
			.exec(document.cookie)) ? f(a[1]) : null
};
...
(function(e) {
	...
	var b = g() + ".mask", d = navigator.userAgent, c = /iphone/i.test(d), a = /android/i
			.test(d), f;
	e.mask = {
		definitions : {
			"9" : "[0-9]",
					...
					if (typeof j == "number") {
						...
						if (this[0].setSelectionRange) {
							...
							if (document.selection
								...
								j = 0 - i.duplicate().moveStart("character",
										-100000);
								h = j + i.text.length
							}
						}
...
(function(c) {
	...
	function j(s, v) {
		var u = typeof s[v];
		return u === "function" || (!!(u == "object" && s[v]))
				|| u == "unknown"
	}
	function k(s, t) {
		return typeof (s[t]) != l
	...
	function h(s) {
		if (window.console && window.console.log) {
			window.console
					.log("TextInputs module for Rangy not supported in your browser. Reason: "
							+ s)
		}
	}
	function o(t, u, s) {
	...
	function r() {
		return e(document, "body") ? document.body : document
	...
	c(document)
			.ready(
					function() {
								...
								d = function(z) {
									...
									if (y && y.parentElement() == z) {
										...
										if (w.compareEndPoints("StartToEnd", A) > -1) {
											...
											C += B.slice(0, C).split("\n").length - 1;
											if (w.compareEndPoints("EndToEnd",
													A) > -1) {
												x = v
											} else {
						...
						i = function(v, y, B) {
							var x = d(v), A = v.value;
							v.value = A.slice(0, x.start) + y + x.text + B
									+ A.slice(x.end);
						...
						function s(v, w) {
							return function() {
								...
								if (z.nodeType == 1
										&& (A == "textarea" || (A == "input" && z.type == "text"))) {
									var y = [ z ].concat(Array.prototype.slice
											.call(arguments));
...
$(function() {
	var a = {
		primaryStyles : [ "fontFamily", "fontSize", "fontWeight",
				"fontVariant", "fontStyle", "paddingLeft", "paddingTop",
				"paddingBottom", "paddingRight", "marginLeft", "marginTop",
				"marginBottom", "marginRight", "borderLeftColor",
				"borderTopColor", "borderBottomColor", "borderRightColor",
				"borderLeftStyle", "borderTopStyle", "borderBottomStyle",
				"borderRightStyle", "borderLeftWidth", "borderTopWidth",
				"borderBottomWidth", "borderRightWidth", "line-height",
				"outline" ],
		specificStyle : {
		...
		toHtml : function(b) {
			return b.replace(/\n/g, "<br>").split(" ").join(
					'<span style="white-space:prev-wrap">&nbsp;</span>')
		},
		getCaretPosition : function() {
			...
			if ($.browser.msie) {
				...
				return {
					...
					top : parseInt(h.boundingTop) - d.top + g.scrollTop
							+ document.documentElement.scrollTop
							+ parseInt(n.getComputedStyle("fontSize"))
				}
			}
			c.simulator.empty();
			...
			var j = $('<span class="before"/>').html(c.toHtml(f)), o = $('<span class="focus"/>'), b = $(
					'<span class="after"/>').html(c.toHtml(m));
			...
			return {
				top : i.top
						- k.top
						- g.scrollTop
						+ ($.browser.mozilla ? 0 : parseInt(n
								.getComputedStyle("fontSize"))),
				left : o[0].offsetLeft - c.simulator[0].offsetLeft
						- g.scrollLeft
			}
		}
	};
			...
			.extend({
				...
				cloneAllStyle : function(e, d) {
					...
					for ( var b in c.style) {
						var f = c.style[b];
						typeof f == "string" || typeof f == "number" ? this
								.cloneStyle(e, b) : NaN
					}
				},
...
(function(f, e, h) {
	...
	f.uaMatch = function(k) {
		k = k.toLowerCase();
		var j = /(opr)[\/]([\w.]+)/.exec(k) || /(chrome)[ \/]([\w.]+)/.exec(k)
				|| /(version)[ \/]([\w.]+).*(safari)[ \/]([\w.]+)/.exec(k)
				|| /(webkit)[ \/]([\w.]+)/.exec(k)
				|| /(opera)(?:.*version|)[ \/]([\w.]+)/.exec(k)
				|| /(msie) ([\w.]+)/.exec(k) || k.indexOf("trident") >= 0
				&& /(rv)(?::| )([\w.]+)/.exec(k) || k.indexOf("compatible") < 0
				&& /(mozilla)(?:.*? rv:([\w.]+)|)/.exec(k) || [];
		var i = /(ipad)/.exec(k) || /(iphone)/.exec(k) || /(android)/.exec(k)
				|| /(windows phone)/.exec(k) || /(win)/.exec(k)
				|| /(mac)/.exec(k) || /(linux)/.exec(k) || /(cros)/i.exec(k)
				|| [];
		return {
			browser : j[3] || j[1] || "",
			version : j[2] || "0",
