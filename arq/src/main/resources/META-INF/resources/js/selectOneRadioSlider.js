/* selectOneRadioSlider.js - @author javier.martinnino */

/* Funcion para transformar los eventos de touch en eventos de raton */
...
/* Definicion del widget del nuevo componente SelectOneRadioSlider */
PrimeFaces.widget.SelectOneRadioSlider = PrimeFaces.widget.Slider;
...
(function(a) {
	if (typeof define === "function" && define.amd) {
		...
		(function(y) {
			 ...
			 * 
			 * Funcion redefinida para adaptarse al componente selectOneRadioSlider
			 */
			var f = y
					.widget(
							"ui.slider",
							y.ui.mouse,
							{
								...
								_create : function() {
									...
									this._mouseInit();
									this.element
											.addClass("ui-selectoneradioslider ui-selectoneradioslider-"
													+ this.orientation
									...
									this.addRadioSliderEvents();
								},
								addRadioSliderEvents : function() {
									document.getElementById(this.options.id).addEventListener("touchstart", touchHandler, true);
									document.getElementById(this.options.id).addEventListener("touchmove", touchHandler, true);
									document.getElementById(this.options.id).addEventListener("touchend", touchHandler, true);
									document.getElementById(this.options.id).addEventListener("touchcancel", touchHandler, true);
								},
								_refresh : function() {
									this._createRange();
									this._createHandles();
								...
								_createHandles : function() {
									var P, M, N = this.options, R = this.element
											.find(".ui-selectoneradioslider-handle")
											.addClass(
													"ui-state-default ui-corner-all"), Q = "<span class='ui-selectoneradioslider-handle ui-state-default ui-corner-all' tabindex='0'></span>", O = [];
									...
									this.handles = R.add(y(O.join(""))
											.appendTo(this.element));
									this.handle = this.handles.eq(0);
									this.handles.each(function(S) {
										y(this).data("ui-selectoneradioslider-handle-index",
												S)
									})
								},
								_createRange : function() {
									var M = this.options, N = "";
									if (M.range) {
										if (M.range === true) {
											if (!M.values) {
												M.values = [ this._valueMin(),
														this._valueMin() ]
											} else {
												if (M.values.length
														&& M.values.length !== 2) {
										...
										if (!this.range || !this.range.length) {
											this.range = y("<div></div>")
													.appendTo(this.element);
											N = "ui-selectoneradioslider-range ui-widget-header ui-corner-all"
										} else {
											this.range
													.removeClass(
															"ui-selectoneradioslider-range-min ui-selectoneradioslider-range-max")
													.css({
														left : "",
														bottom : ""
													})
										...
										this.range
												.addClass(N
														+ ((M.range === "min" || M.range === "max") ? " ui-selectoneradioslider-range-"
																+ M.range
																: ""))
									} else {
										if (this.range) {
								...
								_destroy : function() {
									...
									this.element
											.removeClass("ui-selectoneradioslider ui-selectoneradioslider-horizontal ui-selectoneradioslider-vertical ui-widget ui-widget-content ui-corner-all");
									this._mouseDestroy()
								},
								_mouseCapture : function(O) {
									...
									R = Q.offset();
									M = !y(O.target).parents().addBack().is(
											".ui-selectoneradioslider-handle");
									this._clickOffset = M ? {
										left : 0,
										top : 0
									}
									...
									if (this.options.values
										...
										if (O !== this.value()) {
											...
											if (R !== false) {
												...
												this.options.value = this._trimAlignValue(O);												
												if (this.options.orientation=="horizontal"){
													var left =0;
													left = parseFloat(((event.pageX-this.elementOffset.left-(this.handles[P].clientWidth/2))/this.element.width())*100);
													if (left <0 ){
														left=0;
													}else if (left>100){
														left=100;
													...
													this.handles[P].style.left = left + '%';
												}}
										}else{
								...
								_setOption : function(N, O) {
									...
									case "orientation":
										...
										this.element
												.removeClass(
														"ui-selectoneradioslider-horizontal ui-selectoneradioslider-vertical")
												.addClass(
														"ui-selectoneradioslider-"
																+ this.orientation);
										this._refreshValue();
										break;
									case "value":
								...
								_trimAlignValue : function(P) {
									...
									var M = (this.options.step > 0) ? this.options.step
											: 1, O = (P - this._valueMin()) % M, N = P
											- O;
									if (Math.abs(O) * 2 >= M) {
										N += (O > 0) ? M : (-M)
									}
								...
								_refreshValue : function() {
									var R, Q, U, S, V, P = this.options.range, O = this.options, T = this, N = (!this._animateOff) ? O.animate
											: false, M = {};
									if (this.options.values
											&& this.options.values.length) {
										this.handles
												.each(function(W) {
													Q = (T.values(W) - T
															...
															/ (T._valueMax() - T
																	._valueMin())
															* 100;
													M[T.orientation === "horizontal" ? "left"
															: "bottom"] = Q
															+ "%";
													y(this).stop(1, 1)[N ? "animate"
															: "css"](M,
															O.animate);
										...
										Q = (V !== S) ? (U - S) / (V - S) * 100
												: 0;
										M[this.orientation === "horizontal" ? "left"
												: "bottom"] = Q + "%";
										this.handle.stop(1, 1)[N ? "animate"
												: "css"](M, O.animate);
										if (P === "min"
												&& this.orientation === "horizontal") {
											this.range.stop(1, 1)[N ? "animate"
													: "css"]({
												width : Q + "%"
											}, O.animate)
										}
								...
								_handleEvents : {
									keydown : function(Q) {
										var R, O, N, P, M = y(Q.target).data(
												"ui-selectoneradioslider-handle-index");
										switch (Q.keyCode) {
									...
									keyup : function(N) {
										var M = y(N.target).data(
												"ui-selectoneradioslider-handle-index");
										if (this._keySliding) {
											this._keySliding = false;
											this._stop(N, M);
											this._change(N, M);
											y(N.target).removeClass(
													"ui-state-active")
										}
									}
								}
